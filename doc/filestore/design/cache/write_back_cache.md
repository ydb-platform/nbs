...
These requests go directly to the underlying storage. All prior cached WriteData
requests are flushed and evicted. No new WriteData requests will be flushed
until the direct request is completed.

These requests are implemented as follows:
...
```
ui64 GetMaxWrittenOffset(ui64 nodeId);
```
...
Processing of `WriteData` requests is strictly ordered:
...
![components](../../drawio/WriteBackCacheComponents.svg)
...
- Manages pins;
- Prevents pinned node states from deletion.

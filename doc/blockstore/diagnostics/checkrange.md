...
## Example parser
 - example Python parser can be found in load tests: cloud/blockstore/tests/loadtest/local-checkrange/test.py
  ...
  "Ranges": [
    {
      ...
      "InconsistentChecksums": [ // for mirror disks in case of checksum difference for at least 2 replicas
        {
          "Data": [1, 2, 3, 4, 5]
        }
      ]
...
* for each range of blocks, a checkRange request is sequentially executed. It will be processed by the corresponding actor on the server (for example, the starting point of processing may be part_mirror_actor_checkrange.cpp: HandleCheckRange);
* response processing:
  * checking «high-level» argument processing errors;
  * checking read errors;
  * writing checksums or errors for each required range via TResultManager object;
    * TResultManager builds result in JSON format;
    * each intermediate result is written on the flight without keeping it in memory;
    * result contains checksums for each replica in case of inconsistent mirror checksums;
  * all sequential ProblemRanges are merged into the 1 large for a summary block;
...
* TMirrorPartitionActor::HandleCheckRange:
  * checks that the requested number of blocks is less than the maximum allowed size;
  * registers the actor TMirrorCheckRangeActor, passes the number of replicas to it;
  * TMirrorCheckRangeActor:
    * SendReadBlocksRequest: sends read requests for the specified blocks for each replica of the mirror (TEvReadBlocksRequest).
...
- DoTestShouldCheckRangeChecksumAlgorithm: we check the algorithm of checksums computing
...
- ShouldMirrorCheckRangeOneReadUndelivered: checking the case of undilevered ReadBlocks request.

### Utility tests
All tests are implemented in apps/client/lib/command_ut.cpp
...
It provides tests for replicas inconsistence and result's parsing.

...
| `inflight_dependents` | `Int64` | Number of base disks in `scheduling` or `creating` status whose `src_disk_id` is this base disk. While non-zero, this base disk is not moved to `deleting` by invariants, because the data transfer into those dependents reads from it. Never negative. |
...
A base disk that is used as the source for creating another base disk (it is
the `src_disk_id` of a replacement base disk generated by retirement) must not
be deleted until that replacement is created: `CreateBaseDisk` transfers data
from the source, so deleting the source fails the replacement.

The source is held by its `inflight_dependents` counter, which is the number of
replacement base disks in `scheduling` or `creating` status whose
`src_disk_id` is this base disk. Every base disk transition that puts a disk
with `src_disk_id` into `scheduling`/`creating` increments the counter of its
source, and every transition that takes it out of those statuses (it becomes
`ready`, `creation_failed`, or `deleting` before creation) decrements it
([storage_ydb_impl.go](../../../cloud/disk_manager/internal/pkg/services/pools/storage/storage_ydb_impl.go),
`applyInflightDependents`). The source is appended to the same set of base
disk transitions, so invariants run on it and it is written in the same

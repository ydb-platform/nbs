...
function testLoadSizesAreProgressiveIncrements() {
  ...
  assert.strictEqual(DEFAULT_LOAD_SIZE, 5000);
  ...
  assert.strictEqual(loadSizeFromValue("invalid"), 5000);
...
function testChunkChildCountExcludesPreparationStages() {
  ...
  const spans = [
    makeSpan({
      ...
      name: "tests chunk",
      scope: 0,
      attributes: { "ya.test.result.count": 2 },
    }),
    ...
    makeSpan({
      ...
      name: "chunk",
      scope: 0,
      attributes: { "ya.test.result.count": 1 },
    }),

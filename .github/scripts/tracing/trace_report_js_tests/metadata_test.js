...
function testArtifactLinksUseTheSpanResource() {
  const resources = [
    {
    ...
    {
      "ci.artifact.test_log.url_prefix": "https://reports.example/logs/b/",
      "ci.artifact.test_data.url_prefix": "https://reports.example/data/b/",
    },
  ];
  const first = makeSpan({ id: "first", name: "first", status: 2 });
  const second = makeSpan({ id: "second", name: "second", status: 2 });
  second[RESOURCE] = 1;

  assert.deepStrictEqual(
    artifactLinksForSpan(first, [{}], {
      testLog: "https://legacy.example/logs/",
      testData: "https://legacy.example/data/",
    }),
    {
      testLog: "https://legacy.example/logs/",
      testData: "https://legacy.example/data/",
  ...
  assert.strictEqual(
    artifactLinkForAttribute(
      first,
      "ya.test.log.stderr.path",
      "suite/stderr",
      artifactLinksForSpan(first, resources),
    ),
    "https://reports.example/logs/a/suite/stderr",
  );
  assert.strictEqual(
    artifactLinkForAttribute(
      second,
      "ya.test.logs_directory.path",
      "suite/testing_out_stuff",
      artifactLinksForSpan(second, resources),
    ),
    "https://reports.example/data/b/suite/testing_out_stuff/index.html",
  );
  assert.strictEqual(
    artifactLinkForAttribute(
      first,
      ...
      "suite/stderr",
      artifactLinksForSpan(first, [
        { "ci.artifact.test_log.url_prefix": "javascript:alert(1)" },
      ]),
    ),
    null,
  );
...
for (const test of [
  ...
  testArtifactLinksUseTheSpanResource,
]) {
  test();
}

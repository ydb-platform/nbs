...
function artifactLinksForSpan(span, resources, fallback = {}) {
  const candidate = span && Array.isArray(resources)
    ? resources[span[RESOURCE]]
    : null;
  const resource = candidate && typeof candidate === "object" ? candidate : {};
  const resourcePrefix = (key, fallbackKey) => {
    const value = resource[key];
    return typeof value === "string" && value ? value : fallback[fallbackKey];
  };
  return {
    testLog: resourcePrefix("ci.artifact.test_log.url_prefix", "testLog"),
    testData: resourcePrefix("ci.artifact.test_data.url_prefix", "testData"),
  };
}
    ...
    .forEach(([key, value]) => {
      ...
      const linkTarget =
        ...
        artifactLinkForAttribute(
          ...
          key,
          value,
          artifactLinksForSpan(sourceSpan, model?.r || [], model?.u || {}),
        );
      if (linkTarget !== null) {
...
function spanDetailPanel(index) {
  ...
  const scope = model.c[span[SCOPE]];
  const reportedDuration = reportedTestDurationNs(span, scope);
  const parent =
    span[PARENT] >= 0 ? spans[span[PARENT]][ID] : span[ORPHAN_PARENT] || "none";
  facts.append(
    `Duration: ${formatDuration(reportedDuration ?? span[DURATION])}${
      reportedDuration === null
        ? ""
        : ` reported; ${reportedTestTimingNote(span, scope)}`
    } · Scope: ${scope} · Status: ${span[STATUS]} ${span[STATUS_MESSAGE]}`,

...
function loadSizeFromValue(value) {
  ...
  return LOAD_SIZE_OPTIONS.includes(parsed) ? parsed : DEFAULT_LOAD_SIZE;
}
...
function reportedTestDurationNs(span, scope) {
  if (scope !== "ya.test") return null;
  const seconds = Number(span[ATTRS]["test.duration.reported_seconds"]);
  return Number.isFinite(seconds) && seconds >= 0
    ? Math.round(seconds * 1e9)
    : null;
}

function reportedTestTimingNote(span, scope) {
  if (reportedTestDurationNs(span, scope) === null) return "";
...
function chunkTestCount(index, sourceSpans, sourceChildren, scopes) {
  const reported = Number(
    sourceSpans[index][ATTRS]["ya.test.result.count"],
  );
  if (Number.isInteger(reported) && reported >= 0) return reported;
  return (sourceChildren[index] || []).filter(
...
function directChildCountLabel(index, sourceSpans, sourceChildren, scopes) {
  ...
  if (scope === "ya.chunk") {
    count = chunkTestCount(index, sourceSpans, sourceChildren, scopes);
    ...
    const testCount = chunkChildren.reduce(
      (total, chunk) =>
        total + chunkTestCount(chunk, sourceSpans, sourceChildren, scopes),
      0,
    );

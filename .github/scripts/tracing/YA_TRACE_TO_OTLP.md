...
The implementation is self-contained. Neither its production code nor its tests
import the backed-up v1 [`scripts.tracingv1`](../tracingv1) package. Both
implementations intentionally produce the same OTLP semantics, but their
internal data flow is different.
...
| `subtest-finished` | Test identity, result, duration, errors, logs, and observed finish |
...
2. Test events are assigned to an exact chunk identity when possible. If an
   event lacks a filename or full identity, it is assigned only when there is
   one unambiguous candidate.
3. Events are grouped by `(class, name, type, path)` and split into explicit
   attempts. A new `subtest-started` begins a new attempt.
4. The preferred finish is selected once. `deselected`/`not_launched` records
   do not incorrectly replace a usable finish or reuse a start.
5. Logs, errors, metrics, status code, duration, and all fallback timestamps
...
| `TestAttempt.resolve(...)` | `ya.test` | `<class>::<test>` |
...
### Test attempt interval resolution

`TestAttempt.resolve` records both the interval and its evidence:

| Available evidence | Interval | `test.timing.source` |
| --- | --- | --- |
| Start and finish | Observed timestamps | `subtest-events` |
| Start only | Start to chunk end; error/incomplete | `subtest-start-and-chunk-end` |
| Finish plus duration | Finish minus `time` | `finish-event-and-test-duration` |
| Single test plus delay/startup metrics | Inferred start plus duration | `chunk-delay-and-test-duration` |

Inferred results get `test.timing.inferred=true`; missing finishes also get
`test.incomplete=true`. Test statuses determine OTLP status independently of
the root command status.
...
Ya reports stage durations but not their absolute timestamps. V2 lays these
...
A matched worker becomes the chunk's parent. Worker identity and `test.size`
are copied to the chunk, and the size is propagated to individual tests. The
worker interval is the envelope of the reported worker and chunk intervals;
the unadjusted duration remains in `ya.test.worker.reported_seconds`.
...
the longest remaining nodes fill the budget. Rendered and dropped counts stay
on the operation/root attributes.

### Critical path and longest tests

V2 imports ya's `statistics.critical_path`; it does not recompute the graph's
critical path.
...
Finally, the ten longest complete and launched `ya.test` spans receive
`ya.test.duration.rank=1..10`. This rank is based on reconstructed individual
test intervals, not chunk or worker duration.
...
The stage's `105.0–105.8` placement is inferred from cumulative duration. The
test's `107.0–108.2` interval is observed. The worker expands from its reported
`105.5–109.2` interval to `105.0–109.2` so it contains the chunk.
...
| start `107.0`, finish `108.2` | `TestAttempt(start,finish)` | `107000000000–108200000000` ns |
| `status=good` | `TestEvent(status="good",status_code=1)` | OTLP `OK`, `test.status=good` |
| `time=1.2` | `duration_ns=1200000000` | timing fallback/evidence |
...
An abbreviated standard OTLP proto-JSON test span is shown below. IDs are
illustrative; real IDs are deterministic for the resource, attempt, hierarchy,
and span identity.
...
Public log/test-data base URLs are HTML-only renderer inputs; URLs are inferred
at render time rather than embedded into portable OTLP spans.
...
| [`yatrace/trace_model.py`](yatrace/trace_model.py) | `SuiteTrace`, `Chunk`, `TestAttempt`, `TestEvent`, timing and logs |
| [`yatrace/trace_spans.py`](yatrace/trace_spans.py) | Suite, chunk, test, and inferred test-stage projection |

...
class YaTraceSpanProjector:
    ...
    def _sequence_start(
        ...
        for test_class, name in keys:
            ...
            if any(
                ...
                return None
        total = sum((event.duration_ns for event, _ in attempts), Ns(0))
        if total.value > len(interval):
            return None

        latest = interval.end - total
        ...
        if delay is None:
            return latest
        proposed = interval.start + Ns(max(0, delay.value - startup.value))
        return min(proposed, latest)
    ...
    def _stage_spans(
        ...
        for metric_name, raw_duration in metrics.items():
            ...
            if duration is None or duration.value == 0 or not stage_name:
                continue
            reported_total = reported_total + duration
            end = interval.clamp(cursor + duration)
            if end <= cursor:
                continue
            spans.append(
                stages.make(
    ...
    def _test_spans(
        ...
        for result, incomplete in results:
            ...
            test_interval = (
                Interval(cursor, cursor + result.duration_ns)
                if sequenced
    ...
    def _suite_span(
        ...
        end = max(start, root.clamp(max(timestamps) if timestamps else start))
                ...
                update_span_attributes(
                    chunk,
                    {
                        ...
                        "ya.test.stage.timeline.residual_seconds": Ns(
                            abs(len(interval) - stage_total.value)
                        ).to_s(),
                    },
                )
            if test_failed:

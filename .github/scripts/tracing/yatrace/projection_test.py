...
def test_complete_ya_projection_from_raw_inputs(tmp_path: Path) -> None:
    ...
    assert test_result.start_time_unix_nano == Ns.from_s(6)
    assert test_result.end_time_unix_nano == Ns.from_s(10)
...
def test_single_test_is_right_aligned_without_start_evidence(
    ...
    assert chunk.start_time_unix_nano == Ns.from_s(10)
    assert chunk.end_time_unix_nano == Ns.from_s(12)
    assert span_status_code(chunk) == 2
    assert result.parent_span_id == chunk.span_id
    assert result.start_time_unix_nano == Ns.from_s(11.5)
    assert result.end_time_unix_nano == Ns.from_s(12)
...
def test_nested_results_remain_timestamp_markers(tmp_path: Path) -> None:
    ...
    assert [
        ...
        ("tests::TestParent", Ns.from_s(21), Ns.from_s(21)),
        ("tests::TestParent/child", Ns.from_s(22), Ns.from_s(22)),
    ]
    assert all(

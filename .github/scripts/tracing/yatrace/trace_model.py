...
class TestTiming:
    ...
    def status(self) -> tuple[str, int]:
        return (
            ("incomplete", 2)
            if self.incomplete
            else (self.event.status, self.event.status_code)
        )
    ...
    def resolve(
        ...
        *,
        ...
        if start is not None:
            start_ns = chunk.clamp(
                chunk.start
                if start.record.timestamp_ns is None
                else start.record.timestamp_ns
            )
            if finish is None:
                return TestTiming(
                    Interval(start_ns, chunk.end),
                    start,
                    start.record.order,
                    "subtest-start-and-chunk-end",
                    start.record.timestamp_ns is None,
                    True,
                )
            end_ns = Ns(
                max(
                    start_ns,
                    chunk.clamp(
                        Ns(start_ns + finish.duration_ns)
                        if finish.record.timestamp_ns is None
                        else finish.record.timestamp_ns
                    ),
                )
            )
            return TestTiming(
                ...
                start.record.order,
                "subtest-events",
                start.record.timestamp_ns is None or finish.record.timestamp_ns is None,
            )
        if finish is None:
            return None
        if (
            ...
            and only_test
            and finish.status not in {"deselected", "not_launched"}
        ):
            start_ns = chunk.clamp(inferred_start)
            end_ns = Ns(max(start_ns, chunk.clamp(Ns(start_ns + finish.duration_ns))))
            ...
            start_ns = Ns(max(chunk.start, end_ns - finish.duration_ns))
...
class Chunk:
    ...
    def interval(self, root: Interval) -> Interval:
        ...
        return Interval(start, Ns(max(start, root.clamp(end))))

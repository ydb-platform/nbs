...
#ifndef Y_ABSL_RANDOM_INTERNAL_MOCK_HELPERS_H_
#define Y_ABSL_RANDOM_INTERNAL_MOCK_HELPERS_H_
...
// BitGenRef to enable the mocking capability for y_absl distribution functions.
//
...
//
class MockHelpers {
  using IdType = ::y_absl::base_internal::FastTypeIdType;
  ...
  template <typename KeyT, typename ReturnT, typename ArgTupleT, typename URBG,
            typename... Args>
  static y_absl::optional<ReturnT> InvokeMockImpl(char, URBG*, Args&&...) {
  ...
  static y_absl::optional<ReturnT> InvokeMockImpl(int, URBG* urbg,
                                                Args&&... args) {
    ArgTupleT arg_tuple(std::forward<Args>(args)...);
    ReturnT result;
    if (urbg->InvokeMock(::y_absl::base_internal::FastTypeId<KeyT>(), &arg_tuple,
                         &result)) {
      return result;
    }
    return y_absl::nullopt;
 ...
 public:
  ...
  static auto MaybeInvokeMock(URBG* urbg, Args&&... args)
      -> y_absl::optional<typename KeySignature<KeyT>::result_type> {
  ...
  static auto MockFor(MockURBG& m)
    ...
    return m.template RegisterMock<typename KeySignature<KeyT>::result_type,
                                   typename KeySignature<KeyT>::arg_tuple_type>(
        m, ::y_absl::base_internal::FastTypeId<KeyT>());
  }
};
...
#endif  // Y_ABSL_RANDOM_INTERNAL_MOCK_HELPERS_H_

    ...
    Y_UNIT_TEST(ValidateNullsInNotNullColumns) {
...

        {
            ...
            auto upsert = client.BulkUpsert(
                ...
                .GetValueSync();
            UNIT_ASSERT_C(!upsert.IsSuccess(), upsert.GetIssues().ToString());
            UNIT_ASSERT_STRING_CONTAINS(upsert.GetIssues().ToString(), "not all result columns present");
        }
    ...
    Y_UNIT_TEST(ValidateExplicitDefaultValueColumns) {
...

        {
            ...
            auto upsert = client.BulkUpsert(
                ...
                .GetValueSync();
            UNIT_ASSERT_C(!upsert.IsSuccess(), upsert.GetIssues().ToString());
            UNIT_ASSERT_STRING_CONTAINS(upsert.GetIssues().ToString(), "Cannot read CSV: not all result columns present");
        }
    ...
    Y_UNIT_TEST(DataValidation) {
...

        {
            auto res = client.BulkUpsert(
                ...
                .GetValueSync();
            UNIT_ASSERT_VALUES_EQUAL(res.GetStatus(), EStatus::BAD_REQUEST);
            UNIT_ASSERT_STRING_CONTAINS(res.GetIssues().ToString(), "Invalid JSON for JsonDocument provided: TAPE_ERROR: The JSON document has an improper structure: missing or superfluous commas, braces, missing keys, etc");
        }

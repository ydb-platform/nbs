...
def create_populate_request(args, base_config, destination_pdisk_id, vdisk_ids):
    ...
    skipped_zero_group_vdisks = []
    for vslot in vslots:
        if vslot.GroupId == 0:
            skipped_zero_group_vdisks.append(format_vdisk_id(vslot))
            continue
    ...
    if skipped_zero_group_vdisks:
        print(
            'WARNING: skipped %u VDisks with GroupId=0 (cannot be migrated): %s'
            % (len(skipped_zero_group_vdisks), ', '.join(skipped_zero_group_vdisks[:3])),
            file=sys.stderr,
        )

    if len(cmd.VDiskId) == 0:
        raise Exception('No VDisks to migrate (all VDisks have GroupId=0)')

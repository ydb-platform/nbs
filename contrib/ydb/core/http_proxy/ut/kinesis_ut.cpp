#include <contrib/ydb/core/http_proxy/ut/datastreams_fixture/datastreams_fixture.h>
...
#include <contrib/ydb/library/testlib/service_mocks/iam_token_service_mock.h>
...
static void PatchHistRateBuckets(NJson::TJsonValue& json)
{
...
static void compareJsons(ui16 port, const TString& query, const TString& referenceFile) {
    ...
    Y_UNIT_TEST_F(CreateStreamWithDifferentRetentions, THttpProxyTestMock) {
        ...
        }
        {
            ...
            auto res = SendHttpRequest("/Root", "kinesisApi.CreateStream", request,
                                       FormAuthorizationStr("ru-central1"));
            UNIT_ASSERT_VALUES_EQUAL(res.HttpCode, 400);
            const TString expectedBody("{\"__type\":\"ValidationException\",\"message\":\"<main>: Error: retention hours and storage megabytes must fit one of: { hours : [0, 24],  storage : [0, 0]}, { hours : [0, 168],  storage : [51200, 1048576]}, { hours : [0, 24],  storage : [0, 0]}, { hours : [0, 168],  storage : [51200, 1048576]}, provided values: hours 168, storage 46080, code: 500080\\n\"}");
            UNIT_ASSERT_VALUES_EQUAL(res.Body, expectedBody);
            UNIT_ASSERT_VALUES_EQUAL(res.Description, "ValidationException");
        }
        {

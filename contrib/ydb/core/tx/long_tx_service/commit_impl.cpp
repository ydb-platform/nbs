...
namespace NLongTxService {
    ...
    private:
        ...
        bool SendPrepareTransaction(ui64 tabletId, bool delayed = false) {
            ...
            YDB_LOG_DEBUG("Sending TEvProposeTransaction",
                {"logPrefix", LogPrefix},
        ...
        bool UpdateCoordinators(ui64 tabletId, const google::protobuf::RepeatedField<ui64>& domainCoordinators) {
            ...
            if (!SelectedCoordinator) {
                ...
                YDB_LOG_DEBUG("Selected coordinator",
                    {"logPrefix", LogPrefix},
            ...
            if (!SelectedCoordinator || SelectedCoordinator != privateCoordinator) {
                ...
                YDB_LOG_ERROR("previous coordinator current coordinator",
                    {"logPrefix", LogPrefix},
                    {"error", error},
                    {"selectedCoordinator", SelectedCoordinator},
    ...
    private:
        void PlanTransaction() {
            ...
            YDB_LOG_DEBUG("Sending TEvProposeTransaction",
                {"logPrefix", LogPrefix},
                {"toSelectedCoordinator", SelectedCoordinator});
        ...
        void HandlePlan(TEvTxProxy::TEvProposeTransactionStatus::TPtr& ev) {
            ...
            switch (msg->GetStatus()) {
                ...
                case TEvTxProxy::TEvProposeTransactionStatus::EStatus::StatusConfirmed:
                    ...
                    YDB_LOG_DEBUG("Received TEvProposeTransactionStatus from coordinator",
                        {"logPrefix", LogPrefix},
                ...
                default: {
                    YDB_LOG_ERROR("Received TEvProposeTransactionStatus from coordinator",
                        {"logPrefix", LogPrefix},
                        {"status", msg->GetStatus()});
                    NYql::TIssues issues;
        ...
        bool SendCheckPlannedTransaction(ui64 tabletId, bool delayed = false) {
            ...
            YDB_LOG_DEBUG("Ask TEvProposeTransactionResult",
                {"logPrefix", LogPrefix},
                {"fromColumnShard", tabletId},
                {"planStep", PlanStep},
                {"txId", TxId});

...
Y_UNIT_TEST_SUITE(TColumnShardPublicProtocol) {
    Y_UNIT_TEST(PreserveResultWireFormat) {
        ...
        TEvColumnShard::TEvProposeTransactionResult result(
            42, NKikimrTxColumnShard::TX_KIND_COMMIT, 150,
            NKikimrTxColumnShard::SUCCESS);
        ...
        const TString errorWire(
            "\x08\x05\x10\x02\x18\x2a\x20\x96\x01\x28\x00\x3a\x07timeout", 20);
        TEvColumnShard::TEvProposeTransactionResult error(
            42, NKikimrTxColumnShard::TX_KIND_COMMIT, 150,
            NKikimrTxColumnShard::TIMEOUT, "timeout");
    ...
    Y_UNIT_TEST(PreserveStatusMapping) {
        ...
        for (const auto& [columnShardStatus, ydbStatus] : cases) {
            UNIT_ASSERT_VALUES_EQUAL(
                NColumnShard::ConvertToYdbStatus(columnShardStatus), ydbStatus);
        }
    }
}

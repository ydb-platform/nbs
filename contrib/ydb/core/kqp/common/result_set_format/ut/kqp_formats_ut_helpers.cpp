...
#include <contrib/ydb/core/kqp/common/result_set_format/kqp_formats_arrow.h>

#include <contrib/ydb/library/yql/minikql/mkql_string_util.h>
#include <contrib/ydb/library/yql/minikql/mkql_type_helper.h>
    ...
    const NMiniKQL::TDataType* dataType)
{
    ...
    bool success = SwitchMiniKQLDataTypeToArrowType(dataSlot,
        [&]<typename TType>() {
            result = ExtractDataValue<TType>(array, row, dataSlot);
            return true;
        });
    ...
    const NMiniKQL::THolderFactory& holderFactory)
{
    ...
    switch (itemType->GetKind()) {
        ...
        case NMiniKQL::TType::EKind::Data: {
            return ExtractUnboxedValue(array, row, static_cast<const NMiniKQL::TDataType*>(itemType));
        }

        case NMiniKQL::TType::EKind::Optional: {
            return ExtractUnboxedValue(array, row, static_cast<const NMiniKQL::TOptionalType*>(itemType), holderFactory);
        }

        case NMiniKQL::TType::EKind::Struct: {
            return ExtractUnboxedValue(array, row, static_cast<const NMiniKQL::TStructType*>(itemType), holderFactory);
        }

        case NMiniKQL::TType::EKind::Tuple: {
            return ExtractUnboxedValue(array, row, static_cast<const NMiniKQL::TTupleType*>(itemType), holderFactory);
        }

        case NMiniKQL::TType::EKind::List: {
            return ExtractUnboxedValue(array, row, static_cast<const NMiniKQL::TListType*>(itemType), holderFactory);
        }

        case NMiniKQL::TType::EKind::Dict: {
            return ExtractUnboxedValue(array, row, static_cast<const NMiniKQL::TDictType*>(itemType), holderFactory);
        }

        case NMiniKQL::TType::EKind::Variant: {
            return ExtractUnboxedValue(array, row, static_cast<const NMiniKQL::TVariantType*>(itemType), holderFactory);
        }

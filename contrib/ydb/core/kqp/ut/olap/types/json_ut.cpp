...
#include <contrib/ydb/core/kqp/ut/olap/combinatory/variator.h>
#include <contrib/ydb/core/kqp/ut/olap/helpers/get_value.h>
#include <contrib/ydb/core/kqp/ut/olap/helpers/local.h>
#include <contrib/ydb/core/kqp/ut/olap/helpers/query_executor.h>
#include <contrib/ydb/core/kqp/ut/olap/helpers/typed_local.h>
#include <contrib/ydb/core/kqp/ut/olap/helpers/writer.h>

#include <contrib/ydb/core/base/tablet_pipecache.h>
#include <contrib/ydb/core/formats/arrow/accessor/sub_columns/types.h>
#include <contrib/ydb/core/formats/arrow/serializer/native.h>
#include <contrib/ydb/core/kqp/ut/common/columnshard.h>
...
#include <contrib/ydb/core/tx/limiter/grouped_memory/service/process.h>
...
#include <contrib/ydb/library/yql/minikql/jsonpath/parser/parser.h>
#include <contrib/ydb/library/yql/types/binary_json/format.h>
...
Y_UNIT_TEST_SUITE(KqpOlapJson) {
    ...
    Y_UNIT_TEST_STRING_VARIATOR(Utf8JsonWriting, scriptUtf8JsonWriting) {
        NColumnShard::TTableUpdatesBuilder updates(NArrow::MakeArrowSchema(
            { { "Col1", NScheme::TTypeInfo(NScheme::NTypeIds::Uint64) }, { "Col2", NScheme::TTypeInfo(NScheme::NTypeIds::Utf8) } }));
        updates.AddRow().Add<int64_t>(1).Add("{\"a\" : \"c\"}");
        auto arrowString = Base64Encode(NArrow::NSerialization::TNativeSerializer().SerializeFull(updates.BuildArrow()));
...
// TODO: fix if top-level arrays are needed
    ...
    TString scriptBloomMixIndexesOldSyntaxVariants = R"(
        STOP_COMPACTION
        ------
        SCHEMA:
        CREATE TABLE `/Root/ColumnTable` (
    ...
    Y_UNIT_TEST_STRING_VARIATOR(BloomMixIndexesOldSyntaxVariants, scriptBloomMixIndexesOldSyntaxVariants) {
        Variator::ToExecutor(Variator::SingleScript(__SCRIPT_CONTENT)).Execute();
    }
    ...
    TString scriptBloomNGrammIndexesVariants = R"(
        ...
        ------
        READ: SELECT * FROM `/Root/ColumnTable` ORDER BY Col1;
        EXPECTED: [[1u;["{\"a.b.c\":\"a1\"}"]];[2u;["{\"a.b.c\":\"a2\"}"]];[3u;["{\"b.c.d\":\"b3\"}"]];[4u;["{\"a\":\"a4\",\"b.c.d\":\"b4\"}"]];[10u;#];[11u;["{\"a.b.c\":\"1a1\"}"]];[12u;["{\"a.b.c\":\"1a2\"}"]];[13u;["{\"b.c.d\":\"1b3\"}"]];[14u;["{\"a\":\"a4\",\"b.c.d\":\"1b4\"}"]]]
        ------
        READ: SELECT * FROM `/Root/ColumnTable` WHERE JSON_VALUE(Col2, "$.\"a.b.c\"") = "a1" ORDER BY Col1;
        EXPECTED: [[1u;["{\"a.b.c\":\"a1\"}"]]]
    ...
    TString scriptBloomNGrammIndexesOldSyntaxVariants = R"(
        STOP_COMPACTION
        ------
        SCHEMA:
        CREATE TABLE `/Root/ColumnTable` (
    ...
    Y_UNIT_TEST(CommonJsonScenarios) {
...

        {
            auto result = kikimr.GetQueryClient()
                ...
                `DATA_EXTRACTOR_CLASS_NAME`=`JSON_SCANNER`, `SCAN_FIRST_LEVEL_ONLY`=`false`,
...

        {
            auto result = kikimr.GetQueryClient()
                ...
                VALUES (1, JsonDocument(@@{"a": "b\"b\"", "c": 1, "d": 1.2, "e": true, "f": {"g": {"h": "i"}}, "j": [1, 2]}@@));
    ...
    Y_UNIT_TEST(ScanFirstLevelOnly) {
        auto settings = TKikimrSettings();
        settings.AppConfig.MutableColumnShardConfig()->SetAlterObjectEnabled(true);
        TKikimrRunner kikimr(settings);
        auto client = kikimr.GetTableClient();
...

        {
            auto status = kikimr.GetQueryClient()
                              .ExecuteQuery(R"(
                SELECT JSON_VALUE(json_payload, "$.a") FROM `/Root/olapTable`;
                )",NYdb::NQuery::TTxControl::BeginTx().CommitTx()).GetValueSync();
            UNIT_ASSERT_C(status.IsSuccess(), status.GetIssues().ToOneLineString());
...

        {
            auto status = kikimr.GetQueryClient()
                              .ExecuteQuery(R"(
                SELECT JSON_VALUE(json_payload, "$.a.d") FROM `/Root/olapTable`;
                )",NYdb::NQuery::TTxControl::BeginTx().CommitTx()).GetValueSync();
            UNIT_ASSERT_C(status.IsSuccess(), status.GetIssues().ToOneLineString());
...

        {
            auto status = kikimr.GetQueryClient()
                              .ExecuteQuery(R"(
                SELECT JSON_VALUE(json_payload, "$.a.b.c") FROM `/Root/olapTable`;
                )",NYdb::NQuery::TTxControl::BeginTx().CommitTx()).GetValueSync();
            UNIT_ASSERT_C(status.IsSuccess(), status.GetIssues().ToOneLineString());

import os
import contrib.ydb.tests.olap.load.lib.clickbench as clickbench
import yatest.common
from contrib.ydb.tests.functional.tpc.lib.conftest import FunctionalTestBase


class TestClickbench(clickbench.TestClickbench, FunctionalTestBase):
    iterations: int = 1

    @classmethod
    def setup_class(cls) -> None:
        cls.setup_cluster()
        cls.run_cli(['workload', 'clickbench', '-p', 'olap_yatests/clickbench/hits', 'init', '--store=column'])
        cls.run_cli(['workload', 'clickbench', '-p', 'olap_yatests/clickbench/hits', 'import', 'files', '--input', yatest.common.source_path("contrib/ydb/tests/functional/clickbench/data/hits.csv")])
        os.environ['NO_VERIFY_DATA_CLICKBENCH'] = '1'
        clickbench.TestClickbench.setup_class()

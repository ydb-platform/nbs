...
from contrib.ydb.tests.library.sqs.test_base import IS_FIFO_PARAMS, TABLES_FORMAT_PARAMS
from contrib.ydb.tests.library.sqs.cloud_test_base import YandexCloudSqsTestBase
from contrib.ydb.tests.library.sqs.test_base import get_test_with_sqs_tenant_installation
from contrib.ydb.tests.library.sqs.requests_client import REQUEST_TIMEOUT
...
class TestSqsYandexCloudMode(get_test_with_sqs_tenant_installation(YandexCloudSqsTestBase)):
    @classmethod
    ...
    def test_ymq_expiring_counters(self):
        ...
        def try_check_total_count(expected):
            retry_count = self._retry_attempts(3, 10)
            while retry_count > 0:
                try:
    ...
    def test_dlq_mechanics_in_cloud(self, is_fifo):
        ...
        result_list = self._wait_for_message_body_in_queue(queue1_url, msg_body)
    ...
    def test_queue_counters_are_in_folder(self, tables_format):
        ...
        queue_resource_id = self._get_queue_resource_id(queue_url, queue_name)

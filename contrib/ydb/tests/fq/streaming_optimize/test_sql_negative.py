...
DATA_PATH = yatest.common.source_path("contrib/ydb/tests/fq/streaming_optimize/suites")


@pytest.fixture
def kqp_run(request) -> KqpRun:
    result = KqpRun(
        config_file=os.path.join("contrib/ydb/tests/fq/streaming_optimize/cfg", "app_config.conf"),
        scheme_file=os.path.join("contrib/ydb/tests/fq/streaming_optimize/cfg", "scheme.sql"),

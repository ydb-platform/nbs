...
from contrib.ydb.tests.sql.lib.test_base import TestBase
from contrib.ydb.tests.library.common.wait_for import wait_for
from contrib.ydb.tests.datashard.lib.dml_operations import DMLOperations
from contrib.ydb.tests.datashard.lib.test_pg_base import TestPgBase
from contrib.ydb.tests.datashard.lib.types_of_variables import (
    cleanup_type_name,
    format_sql_value,
    pk_types,
    non_pk_types,
...
class TestSecondaryIndexBase(TestBase):
    def do_test_secondary_index(
        self,
        table_name: str,
    ...
    def do_test_secondary_index_cover(
        self,
        table_name: str,
        pk_types: dict[str, str],
        all_types: dict[str, str],
        ...
        dml.insert(table_name, all_types, pk_types, index, ttl)
        self.select_all_types(table_name, pk_types, index, all_types, dml)

    def select_all_types(self, table_name: str, pk_types: dict[str, str], index: dict[str, str], all_types, dml: DMLOperations):
        rows = dml.query(f"select count(*) as count from {table_name}")
        count_col = rows[0].count
        statements = []
...
class TestSecondaryIndex(TestSecondaryIndexBase):
    ...
    def test_secondary_index_cover(
        ...
        self.do_test_secondary_index_cover(table_name, pk_types, all_types, index, ttl, unique, sync)


class TestPgSecondaryIndex(TestPgBase, TestSecondaryIndexBase):
    @pytest.mark.parametrize(
        "table_name, pk_types, all_types, index, ttl, unique, sync",
        [

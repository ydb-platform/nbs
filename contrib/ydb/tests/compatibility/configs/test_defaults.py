...
class ConfigDefaultValuesComparator:
    ...
    def compare_versions(self) -> str:
        with open(yatest.common.binary_path(f'contrib/ydb/tests/library/compatibility/configs/{self.old_version}')) as f:
            old = json.load(f).get('proto')
        assert old is not None

        with open(yatest.common.binary_path(f'contrib/ydb/tests/library/compatibility/configs/{self.new_version}')) as f:

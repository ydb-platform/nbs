...
#include <contrib/ydb/library/yql/providers/common/config/yql_config_qplayer.h>
...
private:
    ...
    bool ListSandboxFolder(const TStringBuf url, const TStringBuf token, const TPosition& pos, TExprContext& ctx, NJson::TJsonValue& content) {
        try {
            return WithRetry<std::exception>(3, [&]() { return DoListSandboxFolder(url, token, pos, ctx, content); }, [&](const auto& e, int attempt, int attemptCount) { YQL_CLOG(WARN, ProviderConfig) << "Error in loading sandbox folder " << url << ", attempt " << attempt << "/" << attemptCount << ", details: " << e.what(); });

        ...
        switch (Kind) {
            ...
            case NYql::ETypeAnnotationKind::Callable: {
                ...
                for (ui32 i = 0; i < castedType->GetArgumentsSize(); ++i) {
                    ...
                    inplaceArg[2] = NUdf::TUnboxedValuePod(new TYqlTypeResource(exprCtxPtr, castedType->GetArguments()[i].Type));
                    ; // Type
                    inplaceArgs[i] = arg;
                }
                ...
                inplace[3] = NUdf::TUnboxedValuePod(new TYqlTypeResource(exprCtxPtr, castedType->GetReturnType()));
                ; // Result
                return array;
            }
...
template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Data>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Optional>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::List>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Stream>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Tuple>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Struct>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Dict>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Resource>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Tagged>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Variant>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Callable>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

template IComputationNode* WrapSplitType<NYql::ETypeAnnotationKind::Pg>(TCallable& callable, const TComputationNodeFactoryContext& ctx, ui32 exprCtxMutableIndex);

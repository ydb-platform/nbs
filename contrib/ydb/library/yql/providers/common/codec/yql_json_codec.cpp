                                           ...
                                           const NMiniKQL::THolderFactory& holderFactory)
{
    ...
    switch (type->GetKind()) {
        ...
        case TType::EKind::Optional: {
            YQL_ENSURE(json.IsArray() && json.GetArray().size() <= 1, "Unexpected json type (expected array no more than one element, but got " << jsonType << ")");
...
#define INTEGER_CONVERTOR(type, wideType)                                                                        \
    case NUdf::TDataType<type>::Id: {                                                                            \
        YQL_ENSURE(jsonType == EJsonValueType::JSON_INTEGER || jsonType == EJsonValueType::JSON_UINTEGER,        \
        ...
        YQL_ENSURE(intValue >= std::numeric_limits<type>::min() && intValue <= std::numeric_limits<type>::max(), \

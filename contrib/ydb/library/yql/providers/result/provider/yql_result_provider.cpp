    ...
    for (auto& child : columns->Children()) {
        ...
        if (child->IsList()) {
            ...
            if (child->Child(0)->Content() == "prefix") {
                ...
                ctx.AddError(TIssue(ctx.GetPosition(child->Pos()), TStringBuilder() << "Expected 'prefix' or 'auto', but got: " << child->Child(0)->Content()));
                return IGraphTransformer::TStatus::Error;
            }
        }
    }
...
private:
    ...
    TStatus HandleFillOrPull(TExprBase input, TExprNode::TPtr& output, TExprContext& ctx, IDataProvider& provider) {
            ...
            if (forNode.Total().Value()) {
                ...
                if (active.Ref().IsCallable("Void")) {
                    ...
                    if (total == 0) {
                        ...
                        active = ctx.ReplaceNodes(iterLambda.Body().Ptr(), {{iterLambda.Args().Arg(0).Raw(), forNode.World().Ptr()}, {iterLambda.Args().Arg(1).Raw(), listElem}});
                    }
        ...
        if (needWriter && !ResultWriter_) {
            ...
            ResultWriter_->Init(discard, label, Config_->SupportsResultPosition ? TMaybe<TPosition>(ctx.GetPosition(input.Pos())) : Nothing(), unordered);
        }
    ...
    IGraphTransformer::TStatus FinishNode(TExprNode& input, TExprContext& ctx, IGraphTransformer::TStatus status) {
        if (status.Level == TStatus::Ok) {
            ...
            const bool needWriter = input.Content() != TResIf::CallableName() && input.Content() != TResFor::CallableName();
            if (needWriter) {
                                            ...
                                            if (auto maybeWrite = TMaybeNode<TResWrite>(node)) {
                                                ...
                                                for (auto& source : Config_->Types.DataSources) {
                                                    ...
                                                    if (source->CanPullResult(writeInput.Ref(), syncList, canRef)) {
                                                        ...
                                                        if (isRef && !canRef) {
                                                            ctx.AddError(TIssue(ctx.GetPosition(writeInput.Pos()), TStringBuilder() << "RefSelect mode isn't supported by provider: " << source->GetName()));
                                                            return nullptr;
                                                        }
    ...
    TString GetOperationDisplayName(const TExprNode& node) override {
        ...
        if (auto maybeResFor = TMaybeNode<TResFor>(&node)) {
            ...
            if (resFor.Total().Value() && resFor.Total().Value() != "0") {
                res << ", " << (node.GetState() == TExprNode::EState::ExecutionComplete ? resFor.Total().Value() : resFor.Current().Value()) << "/" << resFor.Total().Value();
            }

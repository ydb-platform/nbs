...
import contrib.ydb.library.yql.providers.generic.connector.api.service.connector_pb2_grpc as api
...
def configure_runner(runner, settings) -> Runner:
    if runner is DqRunner:
        return DqRunner(dqrun_path=yat.build_path("contrib/ydb/library/yql/tools/dqrun/dqrun"), settings=settings)
    elif runner is KqpRunner:
        return KqpRunner(kqprun_path=yat.build_path("contrib/ydb/tests/tools/kqprun/kqprun"), settings=settings)
    return None

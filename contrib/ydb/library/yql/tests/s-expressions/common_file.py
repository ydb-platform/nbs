...
def yqlrun_yt_results(provider, prepare, suite, case, config):
    if (suite, case) not in yqlrun_yt_results.cache:
        ...
        yqlrun = YQLRun(prov=provider, keep_temp=True, udfs_dir=yql_binary_path('contrib/ydb/library/yql/tests/common/test_framework/udfs_deps'))
...
def run_test(provider, prepare, suite, case, tmpdir, what):
    ...
    if what == 'RunOnOpt':
        ...
        opt_res, opt_tables_res = execute(
            YQLRun(prov=provider, keep_temp=False, udfs_dir=yql_binary_path('contrib/ydb/library/yql/tests/common/test_framework/udfs_deps')),
            program=res.opt,
            input_tables=in_tables,
            output_tables=out_tables,
            files=files,
    ...
    if what == 'ForceBlocks':
        ...
        blocks_res, blocks_tables_res = execute(
            YQLRun(prov=provider, keep_temp=False, udfs_dir=yql_binary_path('contrib/ydb/library/yql/tests/common/test_framework/udfs_deps'), gateway_config=gateway_config),
            program=program,
            input_tables=in_tables,
            output_tables=out_tables,
            files=files,

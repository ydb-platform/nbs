...
Y_UNIT_TEST_LLVM(TestMinMaxSumTupleKey) {
    ...
    struct TPairHash {
        size_t operator()(const std::pair<ui16, std::string>& p) const {
            return CombineHashes(std::hash<ui32>()(p.first), std::hash<std::string_view>()(p.second));
        }
    };
...
Y_UNIT_TEST_LLVM(TestMinMaxSumTupleKey) {
    ...
    struct TPairHash {
        size_t operator()(const std::pair<ui32, std::string>& p) const {
            return CombineHashes(std::hash<ui32>()(p.first), std::hash<std::string_view>()(p.second));
        }
    };
...
Y_UNIT_TEST_LLVM(TestTpch) {
    ...
    struct TPairHash {
        size_t operator()(const std::pair<std::string_view, std::string_view>& p) const {
            return CombineHashes(std::hash<std::string_view>()(p.first), std::hash<std::string_view>()(p.second));
        }
    };

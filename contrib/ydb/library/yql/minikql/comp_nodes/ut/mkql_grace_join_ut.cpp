...
#include <contrib/ydb/library/yql/minikql/comp_nodes/mkql_grace_join_imp.h>
...
Y_UNIT_TEST_LLVM_SPILLING(TestInnerManyKeyUuid) {
    for (ui32 pass = 0; pass < 1; ++pass) {
        ...
        const auto tupleType1 = pb.NewTupleType({pb.NewDataType(NUdf::TDataType<NUdf::TUuid>::Id),
                                                 pb.NewDataType(NUdf::TDataType<NUdf::TUuid>::Id),
                                                 pb.NewDataType(NUdf::TDataType<NUdf::TUuid>::Id)});

        const auto tupleType2 = pb.NewTupleType({pb.NewDataType(NUdf::TDataType<NUdf::TUuid>::Id),
                                                 pb.NewDataType(NUdf::TDataType<NUdf::TUuid>::Id),
                                                 pb.NewDataType(NUdf::TDataType<NUdf::TUuid>::Id)});
        ...
        const auto resultType = pb.NewFlowType(pb.NewMultiType({pb.NewDataType(NUdf::TDataType<NUdf::TUuid>::Id),
                                                                pb.NewDataType(NUdf::TDataType<NUdf::TUuid>::Id)}));
        ...
        UNIT_ASSERT_EQUAL(u[std::make_pair(
                              ...
                              TString("YYYYYYYYYYYYYYYY"))], 1);
        UNIT_ASSERT_EQUAL(u[std::make_pair(
                              TString("B1B1B1B1B1B1B1B1"),
                              TString("XXXXXXXXXXXXXXXX"))], 1);
        UNIT_ASSERT_EQUAL(u.size(), 2);
    }
}

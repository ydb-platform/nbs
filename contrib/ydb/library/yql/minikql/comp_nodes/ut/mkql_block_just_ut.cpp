#include <contrib/ydb/library/yql/minikql/comp_nodes/mkql_block_just.h>

#include <contrib/ydb/library/yql/minikql/comp_nodes/ut/mkql_block_test_helper.h>
#include <contrib/ydb/library/yql/minikql/comp_nodes/ut/mkql_computation_node_ut.h>
...
Y_UNIT_TEST(NotOptIntTest) {
    ...
    TVector<ui32> input{1, 2, 3, 4};
    TVector<TMaybe<ui32>> expected{1, 2, 3, 4};
    TestJustKernelFuzzied(input, expected);
}

Y_UNIT_TEST(NotOptSingularNullTest) {
    // Test with non-optional TSingularNull
    TVector<TSingularNull> input{TSingularNull(), TSingularNull(), TSingularNull(), TSingularNull()};
    TVector<TMaybe<TSingularNull>> expected{TSingularNull(), TSingularNull(), TSingularNull(), TSingularNull()};
    TestJustKernelFuzzied(input, expected);
}

Y_UNIT_TEST(NotOptTaggedTest) {
    // Test with non-optional tagged values
    auto input = TagVector<TTag::A>(TVector<ui32>{1, 2, 3, 4});
...
Y_UNIT_TEST(OptIntTest) {
    ...
    TVector<TMaybe<ui32>> input{1, 2, Nothing(), 4};
...
Y_UNIT_TEST(OptSingularVoidTest) {
    ...
    TVector<TMaybe<TSingularVoid>> input{TSingularVoid(), TSingularVoid(), Nothing(), TSingularVoid()};
    // The top level TMaybe should always be defined when BlockJust adds a level of optionality
...
Y_UNIT_TEST(OptSingularNullTest) {
    ...
    TVector<TMaybe<TSingularNull>> input{TSingularNull(), TSingularNull(), Nothing(), TSingularNull()};
    // The top level TMaybe should always be defined when BlockJust adds a level of optionality
...
Y_UNIT_TEST(OptTaggedTest) {
    ...
    TVector<TMaybe<TTagged<ui32, TTag::A>>> input{
    ...
    // The top level TMaybe should always be defined when BlockJust adds a level of optionality
    TVector<TMaybe<TMaybe<TTagged<ui32, TTag::A>>>> expected{
...
Y_UNIT_TEST(PgIntTest) {
    ...
    TVector<TPgInt> input{TPgInt(1), TPgInt(2), TPgInt(), TPgInt(4)};
    TVector<TMaybe<TPgInt>> expected{TPgInt(1), TPgInt(2), TPgInt(), TPgInt(4)};
    TestJustKernelFuzzied(input, expected);
}

Y_UNIT_TEST(TaggedOptTest) {
    // Test with tagged optional values (TTagged<TMaybe<ui32>, TTag::A>)
    auto input = TagVector<TTag::A>(TVector<TMaybe<ui32>>{1, 2, Nothing(), 4});

...
Y_UNIT_TEST(TestTuple) {
    ...
    for (size_t sliceIdx = 0; sliceIdx < testSize / sliceSize; sliceIdx++) {
        ...
        for (size_t elemIdx = 0; elemIdx < sliceSize; elemIdx++) {
            ...
            UNIT_ASSERT_VALUES_EQUAL_C(lhs.GetElement(0).Get<i64>(), rhs.GetElement(0).Get<i64>(),
                                       "Expected the same data after trim");
            UNIT_ASSERT_VALUES_EQUAL_C(lhs.GetElement(1).AsStringRef(), rhs.GetElement(1).AsStringRef(),
                                       "Expected the same data after trim");
            UNIT_ASSERT_VALUES_EQUAL_C(bool(lhs.GetElement(2)), bool(rhs.GetElement(2)),
                                       "Expected the same optionality after trim");
            if (bool(lhs.GetElement(2))) {
                UNIT_ASSERT_VALUES_EQUAL_C(lhs.GetElement(2).Get<i64>(), rhs.GetElement(2).Get<i64>(),
                                           "Expected the same data after trim");
            }
        }
    }
    ...
    auto destructorCallsCnt = std::make_shared<int>(0);
    {
        ...
        for (size_t sliceIdx = 0; sliceIdx < testSize / sliceSize; sliceIdx++) {
            ...
            for (size_t elemIdx = 0; elemIdx < sliceSize; elemIdx++) {
                ...
                UNIT_ASSERT_VALUES_EQUAL_C(lhsResource->Get()->Payload, rhsResource->Get()->Payload,
                                           "Expected the same data after trim");
            }
        }
    }

...
template <typename TLeft, typename TRight, typename TOutput>
struct TDateTimeSub: public TSimpleArithmeticBinary<typename TLeft::TLayout, typename TRight::TLayout, typename TOutput::TLayout,
                                                    TDateTimeSub<TLeft, TRight, TOutput>, true> {
    static constexpr auto NullMode = TKernel::ENullMode::Default;
...
struct TAnyDateTimeSubIntervalT {
    ...
    static Value* Generate(Value* left, Value* right, const TCodegenContext& ctx, BasicBlock*& block)
    {
        ...
        const auto wide = SetterFor<typename TOutput::TLayout>(
            GenFromScaledDate<TOutput>(sub, context, block), context, block);
...
void RegisterSub(IBuiltinFunctionRegistry& registry) {
    ...
    RegisterFunctionBinPolyOpt<NUdf::TDataType<NUdf::TInterval>, NUdf::TDataType<NUdf::TInterval>,
                               NUdf::TDataType<NUdf::TInterval>,
                               TIntervalSubInterval,
                               TBinaryArgsOptWithNullableResult>(registry, "Sub");

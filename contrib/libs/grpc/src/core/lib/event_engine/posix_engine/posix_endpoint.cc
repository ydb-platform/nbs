...
uint64_t ParseUlimitMemLockFromFile(TString file_name) {
  static TString kHardMemlockPrefix = "* hard memlock";
  auto result = grpc_core::LoadFile(file_name, false);
  if (!result.ok()) {
    return 0;
  }
  TString file_contents(reinterpret_cast<const char*>((*result).begin()),
                            (*result).length());
    ...
    if (auto dir = opendir("/etc/security/limits.d")) {
      while (auto f = readdir(dir)) {
        ...
        uint64_t hard_memlock = ParseUlimitMemLockFromFile(
            y_absl::StrCat("/etc/security/limits.d/", TString(f->d_name)));
        if (hard_memlock != 0) {
          return hard_memlock;
        }
      }
...
y_absl::Status PosixOSError(int error_no, const char* call_name) {
  y_absl::Status s = y_absl::UnknownError(grpc_core::StrError(error_no));
...
y_absl::Status PosixEndpointImpl::TcpAnnotateError(y_absl::Status src_error) {
...
bool PosixEndpointImpl::TcpDoRead(y_absl::Status& status) {
  struct msghdr msg;
  ...
  do {
    ...
    if (read_bytes <= 0) {
      ...
      if (read_bytes == 0) {
        status = TcpAnnotateError(y_absl::InternalError("Socket closed"));
      } else {
        status = TcpAnnotateError(y_absl::InternalError(
            y_absl::StrCat("recvmsg:", grpc_core::StrError(errno))));
      }
      return true;
    }
  ...
  if (!has_posted_reclaimer_) {
    ...
    memory_owner_.PostReclaimer(
        ...
        [self = Ref(DEBUG_LOCATION, "Posix Reclaimer")](
            y_absl::optional<grpc_core::ReclamationSweep> sweep) {
          if (sweep.has_value()) {
...
void PosixEndpointImpl::UpdateRcvLowat() {
  ...
  if (result.ok()) {
    ...
    gpr_log(GPR_ERROR, "%s",
            y_absl::StrCat("ERROR in SO_RCVLOWAT: ", result.status().message())
                .c_str());
  }
}
...
void PosixEndpointImpl::HandleError(y_absl::Status status) {
  if (!status.ok() ||
...
void PosixEndpointImpl::TcpShutdownTracedBufferList() {
  if (outgoing_buffer_arg_ != nullptr) {
    traced_buffers_.Shutdown(outgoing_buffer_arg_,
                             y_absl::InternalError("TracedBuffer list shutdown"));
    outgoing_buffer_arg_ = nullptr;
  }
}
...
bool PosixEndpointImpl::TcpFlush(y_absl::Status& status) {
  struct msghdr msg;
...
void PosixEndpointImpl::HandleWrite(y_absl::Status status) {
  if (!status.ok()) {
    y_absl::AnyInvocable<void(y_absl::Status)> cb_ = std::move(write_cb_);
    write_cb_ = nullptr;
...
void PosixEndpointImpl::MaybeShutdown(
    y_absl::Status why,
    y_absl::AnyInvocable<void(y_absl::StatusOr<int>)> on_release_fd) {
...
PosixEndpointImpl::PosixEndpointImpl(EventHandle* handle,
  ...
  on_read_ = PosixEngineClosure::ToPermanentClosure(
      [this](y_absl::Status status) { HandleRead(std::move(status)); });
  on_write_ = PosixEngineClosure::ToPermanentClosure(
      [this](y_absl::Status status) { HandleWrite(std::move(status)); });
  on_error_ = PosixEngineClosure::ToPermanentClosure(
      [this](y_absl::Status status) { HandleError(std::move(status)); });

...
// automatic rotation of a customer managed KMS key (https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-cmk)
...
// period of a key that you previously enabled automatic key rotation on. You can
...
// key, use the DisableKeyRotation operation. You can use the GetKeyRotationStatus
// operation to identify any in progress rotations. You can use the
// ListKeyRotations operation to view the details of completed rotations. Automatic
...
// is managed by the Amazon Web Services service that owns the key. In May 2022,
// KMS changed the rotation schedule for Amazon Web Services managed keys from
// every three years (approximately 1,095 days) to every year (approximately 365
// days). New Amazon Web Services managed keys are automatically rotated one year
// after they are created, and approximately every year thereafter. Existing Amazon
// Web Services managed keys are automatically rotated one year after their most
// recent rotation, and every year thereafter. The KMS key that you use for this
...
type EnableKeyRotationInput struct {
    ...
    // date. If no value is specified, the default value is 365 days. The rotation
    // period defines the number of days after you enable automatic key rotation that
    // KMS will rotate your key material, and the number of days between each automatic
    // rotation thereafter. You can use the kms:RotationPeriodInDays (https://docs.aws.amazon.com/kms/latest/developerguide/conditions-kms.html#conditions-kms-rotation-period-in-days)

...
find $FOLDER -name ya.make -o -name 'ya.make.*' -o -name '*.inc' -o -name '*.make' | while read f;do
    if [ "$f" == $FOLDER/public/sdk/python2/ya.make ];then
        ...
        $sed -i -E 's/(^\s*)(ydb\/public\/\b)/\1'$LOC_SED'\/public\//g' $f
    elif [ "$f" == $FOLDER/apps/ya.make ];then
        :
    else
        $sed -i -E 's/(^\s*|^\s*PEERDIR\(\s*|^\s*DEPENDS\(\s*|^\s*EXPORTS_SCRIPT\(\s*|^\s*ADDINCL \s*|^\s*ADDINCL\s*\(\s*GLOBAL \s*|\s*GLOBAL \s*|^\s*PREFIX \s*)(ydb\/\b)/\1'$LOC_SED'\//g;
            ...
            s/\"ydb\/library\/yql\/tools\/mrjob/\"'$LOC_SED'\/library\/yql\/tools\/mrjob/g;
            s/\"ydb\/mvp\//\"'$LOC_SED'\/mvp\//g' $f
    fi
done

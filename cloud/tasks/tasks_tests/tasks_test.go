...
func TestTaskWaitingDurationInChain(t *testing.T) {
	...
	// Top depends on middle, middle depends on bottom

	reqCtx := getRequestContext(t, ctx)
	bottomTaskId, err := scheduleLongTask(reqCtx, s.scheduler)
	require.NoError(t, err)

	reqCtx = getRequestContext(t, ctx)
	middleTaskId, err := scheduleWaitingTask(reqCtx, s.scheduler, bottomTaskId)
	require.NoError(t, err)

	reqCtx = getRequestContext(t, ctx)
	topTaskId, err := scheduleWaitingTask(reqCtx, s.scheduler, middleTaskId)
	require.NoError(t, err)

	timeout := 30 * time.Second

	for _, taskId := range []string{bottomTaskId, middleTaskId, topTaskId} {
		_, err = waitTaskWithTimeout(ctx, s.scheduler, taskId, timeout)
		require.NoError(t, err)
	}

	bottomState, err := s.storage.GetTask(ctx, bottomTaskId)
	require.NoError(t, err)

	middleState, err := s.storage.GetTask(ctx, middleTaskId)
	require.NoError(t, err)

	topState, err := s.storage.GetTask(ctx, topTaskId)
	require.NoError(t, err)

	middleDuration := middleState.WaitingDuration
	topDuration := topState.WaitingDuration
	threshold := 2 * time.Second

	require.EqualValues(t, 0, bottomState.WaitingDuration)
	require.InDelta(t, 10*time.Second, middleDuration, float64(threshold))
	require.InDelta(t, 15*time.Second, topDuration, float64(threshold))
}

...
func (s *TaskState) structValue() persistence.Value {
	return persistence.StructValue(
		...
		persistence.StructFieldValue("running_inflight_for", persistence.IntervalValue(s.RunningInflightFor)),
...
func taskStateTableDescription() persistence.CreateTableDescription {
	return persistence.NewCreateTableDescription(
		...
		persistence.WithColumn("running_inflight_for", persistence.Optional(persistence.TypeInterval)),
...
func (s *storageYDB) scanTaskState(res persistence.Result) (state TaskState, err error) {
	...
	err = res.ScanNamed(
		...
		persistence.OptionalWithDefault("running_inflight_for", &state.RunningInflightFor),

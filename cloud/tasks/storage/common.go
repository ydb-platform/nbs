...
func (s *TaskState) structValue() persistence.Value {
	return persistence.StructValue(
		...
		persistence.StructFieldValue("estimated_inflight_duration", persistence.IntervalValue(s.EstimatedInflightDuration)),
		persistence.StructFieldValue("stalling_duration", persistence.IntervalValue(s.StallingDuration)),
		persistence.StructFieldValue("estimated_stalling_duration", persistence.IntervalValue(s.EstimatedStallingDuration)),
...
func taskStateStructTypeString() string {
	return `Struct<
		...
		estimated_inflight_duration: Interval,
...
func taskStateTableDescription() persistence.CreateTableDescription {
	return persistence.NewCreateTableDescription(
		...
		persistence.WithColumn("estimated_inflight_duration", persistence.Optional(persistence.TypeInterval)),
		persistence.WithColumn("stalling_duration", persistence.Optional(persistence.TypeInterval)),
		persistence.WithColumn("estimated_stalling_duration", persistence.Optional(persistence.TypeInterval)),
...
func (s *storageYDB) scanTaskState(res persistence.Result) (state TaskState, err error) {
	...
	err = res.ScanNamed(
		...
		persistence.OptionalWithDefault("estimated_inflight_duration", &state.EstimatedInflightDuration),
		persistence.OptionalWithDefault("stalling_duration", &state.StallingDuration),
		persistence.OptionalWithDefault("estimated_stalling_duration", &state.EstimatedStallingDuration),

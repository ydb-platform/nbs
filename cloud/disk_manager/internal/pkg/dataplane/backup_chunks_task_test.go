...
func TestBackupChunksTask(t *testing.T) {
	...
	// newStorage keeps chunks in bucket "test" under prefix t.Name().
	...
	entries := []snapshot_storage.BackupQueueEntry{
		{SnapshotID: "snap1", ChunkID: chunkID, Slave: slave.ID},
		// Chunk of a snapshot that has already been deleted.
		{SnapshotID: "snap1", ChunkID: "task.snap1.1", Slave: slave.ID},
	}
	...
	dst, err := slave.S3.GetObject(ctx, slave.Bucket, slave.Key("chunks/"+chunkID))
	require.NoError(t, err)

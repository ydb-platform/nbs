...
func (t *backupSnapshotTask) Run(
	...
	slave, err := t.slaves.Get(t.request.Slave)
	if err != nil {
		return err
	}

	err = t.storage.SetBackupSlave(ctx, snapshotID, slave.ID)
	if err != nil {
		return err
	}
	...
	if !t.state.MetaWritten {
		err = t.writeMeta(ctx, slave, diskID, meta)
		if err != nil {
			return err
		}
	...
	err = t.enqueueChunks(ctx, execCtx, slave)
	if err != nil {
		return err
	}
	...
	err = t.writeMap(ctx, slave, diskID, meta)
	if err != nil {
		return err
	}
...
func (t *backupSnapshotTask) writeMeta(
	...
	return slave.S3.PutObject(
		ctx,
		slave.Bucket,
		slave.Key(layout.MetaObject(diskID, meta.ID)),
...
func (t *backupSnapshotTask) writeMap(
	...
	return slave.S3.PutObject(
		ctx,
		slave.Bucket,
		slave.Key(layout.MapObject(diskID, meta.ID)),
		persistence.S3Object{Data: data},
	)
}

...
func TestSnapshotServiceCreateSnapshotFromDiskWithRootKmsEncryption(t *testing.T) {
	ctx := testcommon.NewContext()
	...
	deleteDisk := func(diskID string) {
		reqCtx := testcommon.GetRequestContext(t, ctx)
		operation, err := client.DeleteDisk(
			reqCtx,
			&disk_manager.DeleteDiskRequest{
				DiskId: &disk_manager.DiskId{
					ZoneId: defaultZoneID,
					DiskId: diskID,
				},
	...
	snapshotResponse := disk_manager.CreateSnapshotResponse{}
	...
	require.NoError(t, err)
	require.Equal(t, int64(diskSize), snapshotResponse.Size)
	...
	reqCtx = testcommon.GetRequestContext(t, ctx)
	operation, err = client.DeleteSnapshot(
		reqCtx,
		&disk_manager.DeleteSnapshotRequest{
			SnapshotId: snapshotID,
		},
	)
	require.NoError(t, err)
...
func TestSnapshotServiceCreateSnapshotFromDiskUseS3(t *testing.T) {
	ctx := testcommon.NewContext()

...
def __fill_fs(client, fs_id):
    ...
    __write_some_data(client, fs_id, "/a0/f6.txt", "123123123123")
...
def test_diff():
    ...
    client.mv("fs1", "/a0/f6.txt", "/a1/b1/f6_moved.txt")
...
def test_unsafe_create_node():
    ...
    stat = json.loads(client.stat(fs_id, "/unsafe-file"))
    assert node_id == stat["Id"]
    assert 0o644 == stat["Mode"]
    assert 10 == stat["Uid"]
    assert 20 == stat["Gid"]

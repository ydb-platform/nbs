...
public:
        ...
        std::shared_ptr<NProto::TReadDataRequest> request) override
    {
        ...
        return DataOps_->ReadData(std::move(callContext), std::move(request))
            .Apply([localBase, shmOffset](const TFuture<NProto::TReadDataResponse>& f) {
...
private:
    void SetupSharedMemory()
    {
        ...
        LocalAddr_ = ::mmap(nullptr, ShmSize_, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        if (LocalAddr_ == MAP_FAILED) {
        ...
        STORAGE_INFO("Shared memory region registered: id=" << RegionId_
            << ", file=" << FileName_
            << ", size=" << ShmSize_);
    }
    ...
    void PingRegion()
    {
        ...
        Inner_->PingMmapRegion(std::move(ctx), std::move(req)).Subscribe(
            [weakSelf = weak_from_this()](const TFuture<NProto::TPingMmapRegionResponse>&) {
                if (auto self = weakSelf.lock()) {
                    self->SchedulePing();
                }
            });
    ...
    ui64 AllocateShmSlot(ui64 /*size*/)
    {
        ...
        ui64 slot = SlotCounter_.fetch_add(1, std::memory_order_relaxed) % NumSlots_;

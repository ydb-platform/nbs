...
#include <cloud/filestore/tools/testing/replay/lib/context.h>
#include <cloud/filestore/tools/testing/replay/lib/executor.h>
#include <cloud/filestore/tools/testing/replay/lib/test.h>
...
public:
    ...
    {
        try {
            const auto future = test->Run();
            if (!future.Initialized()) {
                return;
            }
            const auto& stats = future.GetValueSync();
            if (!stats.GetSuccess()) {

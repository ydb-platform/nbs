...
struct TEnvironment: public NUnitTest::TBaseFixture
{
    ...
    TEnvironment()
        : Log(Logging->CreateLog("INDEX_TEST"))
    {
        RootPath.MkDir();
        StatePath.MkDir();
    }
...
protected:
    bool CreateNestedDir(ui32 pathLen, TMap<TString, ui64>& nodeMap)
    {
        nodeMap.clear();
        ...
        for (ui32 i = 0; i < pathLen; i++) {
            ...
            if (childNode->GetNodeId() <= node->GetNodeId()) {
                ...
                STORAGE_WARN(
                    "node id=" << node->GetNodeId() << " , child node id="
                               << childNode->GetNodeId());
                return false;
            }
    ...
    void SafeCreateReversedNodeIdNestedDir(
        ui32 pathLen,
        TMap<TString, ui64>& nodeMap)
    {
        for (int i = 0; i < 10; i++) {
...
struct NodeLoaderStub: public INodeLoader
{
...
Y_UNIT_TEST_SUITE(TLocalIndex){
    Y_UNIT_TEST_F(ShouldRecoverNestedDir, TEnvironment){ui32 pathLen = 10;
TMap<TString, ui64> nodeMap;
...
Y_UNIT_TEST_F(ShouldDiscardDeletedNodes, TEnvironment)
{
    ...
    auto inserted = index->TryInsertNode(node1, RootNodeId, dir1.GetName());
    ...
    UNIT_ASSERT_C(
        index->LookupNode(node1->GetNodeId()),
        "Failed to lookup  node id: " << node1->GetNodeId()
                                      << ", node: " << dir1.GetName());
    UNIT_ASSERT_C(
        index->LookupNode(node2->GetNodeId()),
        "Failed to lookup  node id: " << node1->GetNodeId()
                                      << ", node: " << dir2.GetName());

    // dir3/dir4 discarded
    UNIT_ASSERT_C(
        !index->LookupNode(node3->GetNodeId()),
        "Did not failed to lookup node id: " << node3->GetNodeId()
                                             << ", node: " << dir3.GetName());
    UNIT_ASSERT_C(
        !index->LookupNode(node4->GetNodeId()),
...
Y_UNIT_TEST_F(ShouldCleanupNodes, TEnvironment)
{
    ...
    auto checkNodeInCache = [&](ui32 nodeIndex, bool isNodeInCache)
    {
        UNIT_ASSERT_VALUES_EQUAL_C(
            ...
            "Node #" << nodeIndex
                     << ", NodeId=" << nodes[nodeIndex]->GetNodeId()
                     << (isNodeInCache ? "doesn't " : " ")
                     << "exist in node cache");
    };

    ...
    ui64 handle) const
{
    ...
    //
    // Only AsyncCreateHandleEnabled qualifies: it is the only mode that reports
    // HandleCreatedAsync, i.e. the only mode where a client knows it has to
    // confirm the handle. TabletUnsafeAsyncReadOnlyCreateHandleEnabled is
    // unsafe exactly because nothing recreates a lost handle there, so waiting
    // for a confirmation that will never arrive would only delay
    // E_FS_BADHANDLE.
    const auto grace = Config->GetUnconfirmedCreateHandleGraceTimeout();
    if (grace
        && Config->GetAsyncCreateHandleEnabled()
        && ctx.Now() < TabletStartTs + grace)
    {
        ui32 flags = 0;

...
{
    if (!AcceptRequest<TEvService::TResolvePathMethod>(
            ev,
            ctx,
            ValidateRequest))
    {
    ...
    ExecuteTx<TResolvePath>(ctx, std::move(requestInfo), msg->Record);
}
    ...
    TTxIndexTablet::TResolvePath& args)
{
    ...
    auto response =
        std::make_unique<TEvService::TEvResolvePathResponse>(args.Error);

...
bool ShouldReadBlobs(const TBlockRangeOverlay& blockRanges)
{
    bool hasBlob = false;
    blockRanges.VisitBlobRanges(
        [&] (const TBlockRangeOverlay::TBlobRange&) {
            hasBlob = true;
        });
    return hasBlob;
}
    ...
    NProtoPrivate::TDescribeDataResponse* record)
{
    ...
    using TBlobRange = TBlockRangeOverlay::TBlobRange;
    ...
    args.BlockRanges.VisitBlobRanges(
        [&] (const TBlockRangeOverlay::TBlobRange& range) {
    ...
    TVector<bool> isFreshBlock(actualBlockCount, false);
    args.BlockRanges.VisitRanges(
        [&] (const TBlockRangeOverlay::TRange& range) {
            if (range.Source.Kind != TBlockRangeOverlay::ESourceKind::Fresh) {
                return;
            }

            for (ui32 k = 0; k < range.BlocksCount; ++k) {
                const ui64 abs = ui64(range.BlockIndex) + k;
                if (abs < actualFirstBlock) {
                    continue;
                }
                const ui64 blockOffset = abs - actualFirstBlock;
                if (blockOffset < isFreshBlock.size()) {
                    isFreshBlock[blockOffset] = true;
                }
            }
        });
...
public:
        ...
        ui32 blocksCount) override
    {
        ...
        if (Args.BlockRanges.AddBlobRange(
                block.BlockIndex,
                blocksCount,
                block.MinCommitId,
                blobId,
                blobOffset))
        {
            // overlay won the comparison for at least part of this range;
            // the corresponding buffer blocks must be cleared so that stale
            // fresh content is not returned for blob-backed blocks
            for (ui64 b = beginBlock; b < endBlock; ++b) {
                Args.Buffer->ClearBlock(b - actualBeginBlock);
            }
        }
    }
    ...
    void Accept(const TBytes& bytes, TStringBuf data) override
    {
        ...
        while (i < bytes.Length) {
            ...
            if (Args.BlockRanges.GetVisibleDataCommitId(
                    IntegerCast<ui32>(
                        Args.ActualRange().FirstBlock() + blockIndex))
                < bytes.MinCommitId)
            {
...
private:
    ...
    const TBlockRangeOverlay BlockRanges;
    TVector<TBlockBytes> Bytes;
    ...
    TTxIndexTablet::TReadData& args)
{
    ...
    if (args.DescribeOnly) {
        ...
        if (readAheadRange.Defined()) {
            ...
            args.BlockRanges = TBlockRangeOverlay(
    ...
    TTxIndexTablet::TReadData& args)
{
    ...
    if (!ShouldReadBlobs(args.BlockRanges)) {
        ApplyBytes(
            LogTag,
            args.ActualRange(),
            std::move(args.Bytes),

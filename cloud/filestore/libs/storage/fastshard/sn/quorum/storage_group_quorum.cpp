...
constexpr ui32 QuorumMirrorGroupType =
    NProtoPrivate::TStorageGroup::E_SG_QUORUM_MIRROR;

// The page size of the config must be a multiple of this.
constexpr ui32 DevicePageSize = 4_KB;
static_assert(sizeof(TStorageGroupHeader) <= DevicePageSize);
...
// Stable across builds, unlike std::hash.
ui64 HashDeviceUUID(TStringBuf deviceUUID)
{
    return CityHash64(deviceUUID.data(), deviceUUID.size());
}

// A page of @p pageSize bytes: the header followed by zeroes.
...
////////////////////////////////////////////////////////////////////////////////
// Client page n lives at device page n + ReservedPages. The shift is applied
// on the way to the device and undone on the way back, so no client request
// can reach the reserved pages.
...
// Fails with E_BADMSG if the device answered with a reserved page.
...
{
    const ui32 pageSize = state->Config.PageSize;
    if (!pageSize || pageSize % DevicePageSize) {
        return MakeError(
            E_ARGUMENT,
            TStringBuilder()
                << "page size " << pageSize << " is not a multiple of "
                << DevicePageSize);
    }

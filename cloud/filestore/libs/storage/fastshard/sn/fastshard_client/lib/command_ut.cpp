...
////////////////////////////////////////////////////////////////////////////////
// IStorageNode whose AcquireDevices cooperatively sleeps until Released is
// set. Used to prove that Shutdown returns from a hung request.

struct TBlockingStorageNode: public IStorageNode
{
        ...
        NCloud::NProto::TAcquireDevicesRequest request) override
    {
        Y_UNUSED(request);
        while (!Released.load()) {
            silk::FiberScheduler::SleepFuture sf;
            silk::FiberScheduler::sleep(10'000'000 /* 10ms */, &sf);
            sf.wait();
        }
...
#define BLOCKING_SN_STUB(name, ...)                                            \
    ...
    BLOCKING_SN_STUB(ReleaseDevices)
    BLOCKING_SN_STUB(ReadPages)
    BLOCKING_SN_STUB(WriteLogRecord)
    BLOCKING_SN_STUB(ReadJournalTail)
    BLOCKING_SN_STUB(AdvanceLsnLowWatermark)

#undef BLOCKING_SN_STUB
};
...
TEST(TFastShardClientTest, ShouldReturnOnShutdownWhileRequestIsInFlight)
{
    ...
    bool result = true;
    std::thread runner([&] {
        result = command->Run(std::size(argv), argv);
    });
    ...
    // node it references go away
    storage->Released.store(true);
    while (!storage->Completed.load()) {

...
void TCreateHandleActor::CreateHandleInShard(const TActorContext& ctx)
{
    ...
    auto request = std::make_unique<TEvService::TEvCreateHandleRequest>();
    request->Record = CreateHandleRequest;
    request->Record.SetFileSystemId(LeaderResponse.GetShardFileSystemId());
...
{
    auto response =
        std::make_unique<TEvService::TEvCreateHandleResponse>(std::move(error));
    NCloud::Reply(ctx, *RequestInfo, std::move(response));
    Die(ctx);
}
...
STFUNC(TCreateHandleActor::StateWork)
{
    switch (ev->GetTypeRewrite()) {
        ...
        HFunc(TEvService::TEvCreateHandleResponse, HandleCreateHandleResponse);

...
private:
    ...
    // Control namespace (".filestore-ctl") - true if the request was
    // answered here and the caller should return, false to proceed as
    // usual. Defined in service_actor_control_namespace.cpp.
    ...
    bool TryHandleControlNamespaceListNodeXAttr(
        const NActors::TActorContext& ctx,
        const TEvService::TEvListNodeXAttrRequest::TPtr& ev,
        const TSessionInfo* session);
    bool TryHandleControlNamespaceSetNodeXAttr(
        const NActors::TActorContext& ctx,
...
private:
    ...
    NActors::IActorPtr CreateUnsafeCreateHandleActor(
        TRequestInfoPtr requestInfo,
        TString input);

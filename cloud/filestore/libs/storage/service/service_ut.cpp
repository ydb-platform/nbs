...
Y_UNIT_TEST_SUITE(TStorageServiceTest)
{
    ...
    void TestZeroCopyWrite(const NProto::TStorageConfig& config, ui64 offset) {
        TTestEnv env({}, config);
        env.CreateSubDomain("nfs");
        ...
        for(auto i = 0; i < iovecsNum; ++i) {
            data.push_back(GenerateValidateData(4_KB, i));
        }
        service.WriteData(headers, fs, nodeId, handle, offset, data);
        for (auto i = 0; i < iovecsNum; ++i) {

...
{
    out << "<form method=\"GET\" id=\"fsSearch\" name=\"fsSearch\">\n"
           "Filesystem: <input type=\"text\" id=\"Filesystem\" "
           "name=\"Filesystem\"/>\n"
...
{
    out << "<a href='../tablets?TabletID=" << tabletId << "'>" << fsId
        << "</a>";
}
    ...
    HTML (out) {
        TABLE_SORTABLE_CLASS("table table-bordered")
        {
            ...
            for (const auto& [_, info]: State->GetLocalFileStores()) {
                TABLER () {
                    TABLED () {
                        out << "<a href='../tablets?TabletID=" << info.TabletId
                            << "'>" << info.FileStoreId << "</a>";
                    }
                    TABLED () {
                        out << "<a href='../tablets?TabletID=" << info.TabletId
                    ...
                    TABLED () {
                        out << FormatByteSize(
                            info.Config.GetBlocksCount() *
                            info.Config.GetBlockSize());
                    }
                    TABLED () {

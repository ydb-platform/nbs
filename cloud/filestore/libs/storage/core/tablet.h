    ...
    public:
            ...
            const NActors::TActorContext& ctx) override
        {
            ...
            if (!TTx::PrepareTx(*Self, ctx, tx, Args)) {
                ...
                // was triggered during PrepareTx() call. In production a a no-op
                // stub is used; tests inject an implementation to make the transaction
                // restart/reorder path.
                if (Self->TxRescheduler->IsTriggered()) {

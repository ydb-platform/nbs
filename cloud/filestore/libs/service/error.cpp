...
{
    return MakeError(MAKE_FILESTORE_ERROR(NProto::E_FS_NOTSUPP), message);
}
...
NProto::TError ErrorInvalidNodeType(ui64 nodeId)
{
    return MakeError(
        ...
        TStringBuilder() << "invalid node type #" << nodeId);
}
...
NProto::TError ErrorInvalidParent(ui64 nodeId)
{
    return MakeError(
        ...
        TStringBuilder() << "invalid parent node #" << nodeId);
}
...
NProto::TError ErrorInvalidTarget(ui64 nodeId, const TString& name)
{
    ...
    return MakeError(MAKE_FILESTORE_ERROR(NProto::E_FS_NOENT), std::move(ss));
}
...
NProto::TError ErrorAlreadyExists(const TString& path)
{
    return MakeError(
        E_FS_EXIST,
        TStringBuilder() << "path " << path.Quote() << " already exists");
}
...
NProto::TError ErrorIsDirectory(ui64 nodeId)
{
    return MakeError(
        ...
        TStringBuilder() << "directory node #" << nodeId);
}
...
NProto::TError ErrorIsNotDirectory(ui64 nodeId)
{
    return MakeError(
        ...
        TStringBuilder() << "not a directory node #" << nodeId);
}
...
NProto::TError ErrorIsNotEmpty(ui64 nodeId)
{
    return MakeError(
        ...
        TStringBuilder() << "not empty directory node #" << nodeId);
}
...
NProto::TError ErrorNameTooLong(const TString& name)
{
    return MakeError(
        ...
        TStringBuilder() << "name too long " << MessageSafeStr(name));
}
...
NProto::TError ErrorMaxLink(ui64 nodeId)
{
    return MakeError(
        ...
        TStringBuilder() << "max link count for node #" << nodeId);
}
...
NProto::TError ErrorFileTooBig()
{
    return MakeError(
        ...
        TStringBuilder() << "file size too big");
}
...
NProto::TError ErrorInvalidArgument()
{
    return MakeError(
        ...
        TStringBuilder() << "invalid arguments specified");
}
...
NProto::TError ErrorInvalidArgument(const TString& reason)
{
    return MakeError(
        MAKE_FILESTORE_ERROR(NProto::E_FS_INVAL),
        TStringBuilder() << "invalid arguments specified, reason: " << reason);
}
...
NProto::TError ErrorInvalidHandle(ui64 handle)
{
    return MakeError(
        MAKE_FILESTORE_ERROR(NProto::E_FS_BADHANDLE),
        TStringBuilder() << "invalid handle #" << handle);
}
...
NProto::TError ErrorAttributeDoesNotExist(const TString& name)
{
    return MakeError(
        ...
        TStringBuilder() << "attribute does not exist " << name.Quote());
}
...
NProto::TError ErrorAttributeAlreadyExists(const TString& name)
{
    return MakeError(
        ...
        TStringBuilder() << "attribute already exists " << name.Quote());
}
...
NProto::TError ErrorAttributeNameTooLong(const TString& name)
{
    return MakeError(
        ...
        TStringBuilder() << "attribute name " << MessageSafeStr(name)
                         << "too big");
}
...
NProto::TError ErrorAttributeValueTooBig(const TString& name)
{
    return MakeError(
        ...
        TStringBuilder() << "attribute " << MessageSafeStr(name)
                         << " value too large");
}
...
NProto::TError ErrorInvalidAttribute(const TString& name)
{
    return MakeError(
        ...
        TStringBuilder() << "invalid attribute name " << name.Quote());
}
    ...
    ui64 sessionSeqNo)
{
    return MakeError(
        ...
        TStringBuilder() << "invalid session ("
...
NProto::TError ErrorInvalidCheckpoint(const TString& checkpointId)
{
    return MakeError(
        ...
        TStringBuilder() << "invalid checkpoint " << checkpointId.Quote());
}
...
NProto::TError ErrorIncompatibleLocks()
{
    return MakeError(
        ...
        TStringBuilder() << "incompatible locks");
}
...
NProto::TError ErrorIncompatibleLockType(int type)
{
    return MakeError(
        MAKE_FILESTORE_ERROR(NProto::E_FS_INVAL),
        TStringBuilder() << "incompatible lock type " << type);
}
...
NProto::TError ErrorIncompatibleLockWhence(int whence)
{
    return MakeError(
        MAKE_FILESTORE_ERROR(NProto::E_FS_INVAL),
        TStringBuilder() << "incompatible whence " << whence);
}
...
NProto::TError ErrorIncompatibleLockOriginLocks()
{
    return MakeError(
        ...
        TStringBuilder() << "incompatible lock origin (flock/fcntl)");
}
...
NProto::TError ErrorIncompatibleFileOpenMode()
{
    return MakeError(
        MAKE_FILESTORE_ERROR(NProto::E_FS_INVAL),
        TStringBuilder() << "incompatible file open mode");
}
...
NProto::TError ErrorDuplicate()
{
    return MakeError(
        E_REJECTED,
        TStringBuilder() << "non-idempotent request is being processed");
}
...
int FileStoreErrorToErrno(int error)
{
    auto it = FindIf(
        SystemErrors.begin(),
        SystemErrors.end(),
        [&](const auto& pair) { return pair.first == error; });
...
int ErrnoToFileStoreError(int error)
{
    auto it = FindIf(
        SystemErrors.begin(),
        SystemErrors.end(),
        [&](const auto& pair) { return pair.second == error; });

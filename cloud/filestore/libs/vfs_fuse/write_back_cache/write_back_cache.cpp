...
struct TWriteBackCache::THandleState
{
    ...
    // Count of entries from TWriteBackCache::TImpl::PendingEntries
    ...
    // Prevent from concurrent read and write requests on overlapping ranges
        ...
        std::shared_ptr<NProto::TReadDataRequest> request)
    {
            ...
            [ptr = weak_from_this(), handle, offset, end](const auto&)
        {
            if (auto self = ptr.lock()) {
                ...
                if (handleState->CanBeRemoved()) {
                    self->HandleStates.erase(handle);
                }
...
private:
    ...
    bool PrepareFlushOperation(THandleState* handleState)
    {
        ...
        with_lock (Lock) {
            if (!handleState->ShouldFlush()) {
                ...
                if (handleState->CanBeRemoved()) {
                    HandleStates.erase(handleState->Handle);
                }
                return false;
            }
    ...
    void CompleteFlushOperation(THandleState* handleState)
    {
        ...
        if (handleState->CanBeRemoved()) {
            HandleStates.erase(handleState->Handle);
        } else if (handleState->ShouldFlush()) {
            ScheduleFlushIfNeeded(handleState);
        }
...
bool TWriteBackCache::TWriteDataEntry::FlushRequested() const
{

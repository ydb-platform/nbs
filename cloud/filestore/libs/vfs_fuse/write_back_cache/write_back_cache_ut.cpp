...
struct TTestTimer: public ITimer
{
...
struct TBootstrap
{
        ...
        , ZeroCopyWriteEnabled(args.ZeroCopyWriteEnabled)
    {
        ...
        Session->WriteDataHandler = [&](auto, auto request)
        {
            ...
            STORAGE_INFO(
                "Flushing " << request->GetBuffer().Quote() << " to @"
                            << request->GetNodeId() << " at offset "
                            << request->GetOffset());
    ...
    ReadFromCache(ui64 nodeId, ui64 handle, ui64 offset, ui64 length)
    {
    ...
    TFuture<NProto::TReadDataResponse>
    ReadFromCache(ui64 nodeId, ui64 offset, ui64 length)
    {
    ...
    void ValidateCache(ui64 nodeId, ui64 offset, size_t length)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL_C(
            ...
            TStringBuilder() << " while validating @" << nodeId << " at offset "
        ...
        const TIovecs& iovecs)
    {
        ...
        future.Subscribe(
            [&, nodeId, offset, buffer](auto)
            {
                STORAGE_INFO(
                    "Written " << buffer.Quote() << " to @" << nodeId
                               << " at offset " << offset);
    ...
    WriteToCache(ui64 nodeId, ui64 offset, TString buffer)
    {
        auto handle = nodeId + NodeToHandleOffset;
    ...
    TFuture<NProto::TWriteDataResponse>
    WriteToCache(ui64 nodeId, ui64 offset, const TIovecs& iovecs)
    {
        auto handle = nodeId + NodeToHandleOffset;
...
TFuture<T> CompleteFutureOnTrigger(TFuture<T> future, TFuture<void> trigger)
{
    ...
    trigger.Subscribe(
        ...
        {
            future.Subscribe([promise = std::move(promise)](auto f) mutable
...
Y_UNIT_TEST_SUITE(TWriteBackCacheTest)
{
    Y_UNIT_TEST(ShouldReadEmptyCache)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL("", readFuture.GetValue().GetBuffer());
    }
            ...
            threads.emplace_back(
                [&, nodeId = i]
                {
                    ...
                    while (writesRemaining--) {
                        ...
                        auto data =
                            TStringBuf(alphabet).SubString(offset, length);
            ...
            threads.emplace_back(
                [&]
                {
                    ...
                    while (readsRemaining--) {
                        ...
                        auto future =
                            b.Cache.ReadData(b.CallContext, move(request));
                        auto response = future.GetValueSync();
                    }
                });
        }
    ...
    Y_UNIT_TEST(ShouldLimitTotalWriteRequestSizeAtFlush)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL("(0, 6), (6, 4)", logger.RangesToString(1));
    }
    ...
    Y_UNIT_TEST(ShouldShareCacheBetweenHandles)
    {
        ...
        UNIT_ASSERT(readFuture.HasValue());
        UNIT_ASSERT_VALUES_EQUAL("ade", readFuture.GetValue().GetBuffer());
    }

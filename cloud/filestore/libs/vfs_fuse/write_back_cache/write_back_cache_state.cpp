...
struct TNodeState
{
    ...
    ui64 GetMinPendingOrUnflushedSequenceId(ui64 defValue) const
    {
        ...
        if (!PendingRequestQueue.empty()) {
            return PendingRequestQueue.front()->GetSequenceId();
        }
        return defValue;
    }
};
    ...
    void FlushFailed(ui64 nodeId, const TError& error)
    {
        ...
        if (nodeState.Handles.size() == nodeState.HandleReleaseCount) {
            ...
            for (auto& it: nodeState.PendingRequestQueue) {
                Listener.Fail(std::move(*it->MutablePromise()), error);
                RequestManager.Remove(std::move(it));
            }
...
private:
        ...
        std::unique_ptr<TPendingWriteDataRequest> request)
    {
        ...
        nodeState.Handles[request->GetRequest().GetHandle()].Count++;
    ...
    void EvictUnpinnedFlushedEntries(ui64 nodeId, TNodeState& nodeState)
    {
        ...
        while (RequestManager.HasPendingRequests()) {
            ...
            Y_ABORT_UNLESS(
                nodeState.PendingRequestQueue.front()->GetSequenceId() ==
                request->GetSequenceId());

            Listener.Complete(
                std::move(
                    *nodeState.PendingRequestQueue.front()->MutablePromise()));

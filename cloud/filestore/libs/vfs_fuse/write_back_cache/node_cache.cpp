...
{
    return UnflushedRequests.empty() ? 0
...
{
    return FlushedRequests.empty() ? 0 : FlushedRequests.front()->GetSequenceId();
}
...
void TNodeCache::VisitCachedData(
    ui64 offset,
    ui64 byteCount,
    const TCachedDataVisitor& visitor) const
{
    IntervalMap.VisitOverlapping(
        offset,
        offset + byteCount,
        [&visitor, offset, end = offset + byteCount](auto it)
        {
            ...
            TStringBuf data = entry->GetCachedData(partOffset, partByteCount);
            return visitor({partOffset, data});
        });
}

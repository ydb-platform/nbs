...
Y_UNIT_TEST_SUITE(TWriteDataRequestBuilderTest)
{
    Y_UNIT_TEST(ShouldCalculateEntriesCountToFlush)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL(1, b.CalculateEntriesCountToFlush(
            singleEntry, 100, 100, 1000));

        UNIT_ASSERT_VALUES_EQUAL(1, b.CalculateEntriesCountToFlush(
            singleEntry, 1, 2, 1000));

        UNIT_ASSERT_VALUES_EQUAL(1, b.CalculateEntriesCountToFlush(
            singleEntry, 100, 100, 2));


        TTestCaseWriteDataEntries twoOverlappingEntries{{1, 0, 3}, {1, 1, 3}};

        UNIT_ASSERT_VALUES_EQUAL(2, b.CalculateEntriesCountToFlush(
            twoOverlappingEntries, 100, 100, 1000));

        UNIT_ASSERT_VALUES_EQUAL(2, b.CalculateEntriesCountToFlush(
            twoOverlappingEntries, 100, 100, 4));

        UNIT_ASSERT_VALUES_EQUAL(1, b.CalculateEntriesCountToFlush(
            twoOverlappingEntries, 100, 100, 3));

        UNIT_ASSERT_VALUES_EQUAL(2, b.CalculateEntriesCountToFlush(
            twoOverlappingEntries, 100, 1, 1000));

        UNIT_ASSERT_VALUES_EQUAL(2, b.CalculateEntriesCountToFlush(
            twoOverlappingEntries, 3, 100, 1000));


        TTestCaseWriteDataEntries twoSeparateEntries{{1, 0, 3}, {1, 4, 3}};

        UNIT_ASSERT_VALUES_EQUAL(2, b.CalculateEntriesCountToFlush(
            twoSeparateEntries, 100, 100, 1000));

        UNIT_ASSERT_VALUES_EQUAL(1, b.CalculateEntriesCountToFlush(
            twoSeparateEntries, 100, 100, 4));

        UNIT_ASSERT_VALUES_EQUAL(1, b.CalculateEntriesCountToFlush(
            twoSeparateEntries, 100, 1, 1000));

        UNIT_ASSERT_VALUES_EQUAL(2, b.CalculateEntriesCountToFlush(
            twoSeparateEntries, 3, 100, 1000));
    }

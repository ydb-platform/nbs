...
private:
    ...
    // also protected by |WriteDataRequestsQueueLock|
    ...
    TMutex WriteDataRequestsQueueLock;
...
public:
    ...
    // should be protected by |WriteDataRequestsQueueLock|
        ...
        TString* out)
    {
        ...
        with_lock (WriteDataRequestsQueueLock) {
            const auto parts = CalculateDataPartsToRead(
                handle,
                startingFromOffset,
                length);

            for (const auto& part: parts) {
                ReadDataPart(part, startingFromOffset, out);
            }
        }
    }
        ...
        std::shared_ptr<NProto::TReadDataRequest> request)
    {
        ...
        return Session->ReadData(std::move(callContext), request).Apply(
            [ptr = weak_from_this(),
             ...
             length = request->GetLength()] (auto future)
            {
    ...
    void EraseFlushedEntries(const TVector<TWriteDataEntry*> entries)
    {
        with_lock (WriteDataRequestsQueueLock) {
            ...
            while (!WriteDataEntries.Empty()) {
                ...
                Erase(entries, entry);
                if (entries.empty()) {
                    WriteDataEntriesByHandle.erase(
                    entry->Handle);
                }
    ...
    TFuture<void> FlushData(ui64 handle)
    {
        struct TState
        {
            ...
            std::atomic<int> RequestsRemaining = 0;
        };
        ...
        with_lock (WriteDataRequestsQueueLock) {
            ...
            entries = WriteDataEntriesByHandle[handle];
        }

        auto state = std::make_shared<TState>();
        state->RequestsRemaining = requests.size();
        ...
        future.Subscribe(
             ...
             entries = std::move(entries)] (auto)
            {
                ...
                self->EraseFlushedEntries(entries);
            });
        return future;
    }
    ...
    TFuture<void> FlushAllData()
    {
        ...
        auto state = std::make_shared<TState>();
        state->RequestsRemaining = handles.size();
        ...
        future.Subscribe([ptr = weak_from_this()] (auto) {
            ...
            with_lock (self->WriteDataRequestsQueueLock) {
                while (!self->PendingWriteDataRequests.Empty()) {
                    ...
                    if (self->WriteDataRequestsQueue.Push(result)) {
                        ...
                        std::unique_ptr<TPendingWriteDataRequest> holder(
                            pending);
                        pending->Unlink();
                    } else {

...
Y_UNIT_TEST_SUITE(TFileRingBufferTest)
{
    ...
    FILE_RING_BUFFER_TEST(ShouldValidate)
    {
        ...
        if (ver >= EVersion::V6) {
            // In V6, checksum is additionaly XOR-ed with the header checksum
    ...
    FILE_RING_BUFFER_TEST(ShouldGetVersion)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL(static_cast<int>(ver), rb.GetVersion());
    }
    ...
    Y_UNIT_TEST(ShouldGetMaxObservedEntryByteCount)
    {
        const auto f = TTempFileHandle();
        const ui32 len = 36;
        auto rb = std::make_unique<TFileRingBuffer>(f.GetName(), len, 0, EVersion::V5);
        ...
        rb = std::make_unique<TFileRingBuffer>(f.GetName(), len, 0, EVersion::V5);
        UNIT_ASSERT_EQUAL(5, rb->GetMaxObservedEntryByteCount());
    }

...
`IDevice` (`iface/device.h`) is the raw device: `ReadPages` and `WritePages`, both
addressing whole pages by number. `CreateInMemoryDevice` (`memory_device.h`) is the
test implementation.
...
| `ReadJournalTail` | the records past the acked lsn that the journal still holds, in chain order |
...
There are two implementations. `CreateJournalledDevice`
(`journalled_device_v1.cpp`) is the v1 pass-through, with no journal at all: it
forwards reads and writes to the device, refusing only a write whose `Lsn` is
not above its `PrevLsn` (`E_ARGUMENT`) - the lsn order check is disabled for
now. `AdvanceLsnLowWatermark` just remembers the watermark in memory, and
`ReadJournalTail` returns no records, only that watermark as the last acked
lsn.
...
// restores the journal and starts the flush cycle; blocks until the restore
// is done and throws if it fails
device->Start();
```
...
metadata (`TJournalMetadata`: a format version and `LastAckedLsn`).
...
`Acquire` pins the current value, `AcquireAtLeast` pins the higher of the
current value and the given lsn; `GetBarrierLsn` returns the lower of the
current value and the lowest pinned one. Two of them are used:
...
- every other page is an **entry page**: a 48-byte header (magic, seq, key,
  payload size, version, chunk index, chunk count, crc32c over the header and
  the chunk) followed by a chunk of the value. A value larger than one chunk
  spans several pages, which need not be contiguous - the pages come from a
...
`TJournal` ties the chain, the index and the two stores together. It is meant
to run on the coroutine `TExecutor`: the methods that talk to the stores wait
on their futures with `Executor->WaitFor`, so they must be called from an
executor thread.

## The paths

Every request to `TJournalledDeviceV2` must carry the `deviceUUID` the device
was built with, otherwise it is answered `E_ARGUMENT` before reaching the
executor.

Reads and writes both require the page ranges of a single request to be
disjoint. The journal checks this before touching anything and answers
`E_ARGUMENT`: a page covered by two ranges of one request has no single answer -
on a read it would be looked up twice and taken from the merged response twice,
on a write it would be mapped twice, leaving the winner to the order the groups
happen to be in and handing the tail two groups claiming the same page. Ranges
...
3. The record is inserted into the chain. A record whose `PrevLsn` is not below
   its `Lsn` is rejected with `E_ARGUMENT`, an overlapping one with
   `E_INVALID_STATE`; a duplicate returns the held record's future.
...
`TJournalledDeviceV2::DoReadPages` pins the indexed barrier, asks the journal,
computes the gaps (`MakeMissingRequest`), reads those from the data device, and
stitches the two responses together in the shape of the request
(`MergeResponses`). A page missing from both is an `E_INVALID_STATE`.
...
up to `MaxRecordCount` records when it is set, reading each record's payload
back out of the log data store. This is how a writer that restarts finds out
...
2. The record's page groups are written to the data device at their real page
   ...
   `backgroundClientId` the device was built with - the disk agent refuses a
   write without a device UUID and checks access by the client id. A record
   that maps no pages is a chain link and nothing more, so it is marked flushed
   without a device request at all.
3. When the loop ends - nothing is left to flush, or a flush failed and will be
   retried on the next cycle - `CleanupFlushedRecords` erases the
...
`TJournalledDeviceV2` runs the work of every public call on the coroutine
`TExecutor`, so both it and the journal below it run on executor threads and
may block on futures there. `Start` and `Stop` block the calling thread until
that work is done. The stores are free-threaded: their state is under a lock
and the device requests are issued outside of it. The chain, the index and the
barriers each guard themselves with a spinlock.
...
| file | what it covers |
| --- | --- |
| `memory_device_ut.cpp` | the in-memory device |
...
| `log_record_ut.cpp` | journal metadata (de)serialization |
...
| `journalled_device_v1_ut.cpp` | the v1 pass-through device |

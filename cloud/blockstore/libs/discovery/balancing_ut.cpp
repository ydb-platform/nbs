...
Y_UNIT_TEST_SUITE(BalancingTest)
{
    Y_UNIT_TEST(ShouldCalculateBalancingScoreAndReorder)
    {
        ...
        UNIT_ASSERT_DOUBLES_EQUAL(10, instances.Instances[0].BalancingScore, 1e-5);

        UNIT_ASSERT_VALUES_EQUAL("newhost", instances.Instances[1].Host);
        UNIT_ASSERT_DOUBLES_EQUAL(1, instances.Instances[1].BalancingScore, 1e-5);

        UNIT_ASSERT_VALUES_EQUAL("host2", instances.Instances[2].Host);
        UNIT_ASSERT_DOUBLES_EQUAL(0.01, instances.Instances[2].BalancingScore, 1e-5);

        UNIT_ASSERT_VALUES_EQUAL("host1", instances.Instances[3].Host);
        UNIT_ASSERT_DOUBLES_EQUAL(0.005, instances.Instances[3].BalancingScore, 1e-5);

        UNIT_ASSERT_VALUES_EQUAL("unreachablehost", instances.Instances[4].Host);
        UNIT_ASSERT_DOUBLES_EQUAL(0, instances.Instances[4].BalancingScore, 1e-5);

        UNIT_ASSERT_VALUES_EQUAL("newesthost", instances.Instances[5].Host);
        UNIT_ASSERT_DOUBLES_EQUAL(0, instances.Instances[5].BalancingScore, 1e-5);
    }
}

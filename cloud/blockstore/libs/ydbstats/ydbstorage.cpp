    ...
    TTableClient::TOperationFunc describe = [=] (TSession session) {
        return session.DescribeTable(tableName).Apply([=] (const auto& future) mutable {
            if (future.GetValue().IsSuccess()) {
                ...
                for (const auto& column : description.GetPrimaryKeyColumns()) {
                    keyColumns.push_back(TString{column});
                }

                auto ttlSettings = description.GetTtlSettings();

                result.SetValue(TDescribeTableResponse(
                    columns,
                    keyColumns,
                    ttlSettings ? MakeMaybe(*ttlSettings) : Nothing()));
            }
        ...
        [=, this] (const auto& future) {
            ...
            for (const auto& item: tables) {
                if (item.Type == ESchemeEntryType::Table) {
                    auto creationTime = ExtractTableTime(TString{item.Name});
                    if (creationTime) {

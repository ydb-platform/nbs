...
public:
        ...
        const NYdb::NTable::TTableDescription& description) override
    {
        ...
        for (const auto& key : description.GetPrimaryKeyColumns()) {
            keyColumns.push_back(TString{key});
        }

        auto scheme = std::make_shared<TStatsTableScheme>(
            columns,
            keyColumns,
            description.GetTtlSettings() ? MakeMaybe(*description.GetTtlSettings()) : Nothing());
        ...
        if (it == Tables.end()) {
            ...
            Tables.insert({
                ...
                std::make_shared<TStatsTableScheme>(
                    origColumns,
                    origKeyColumns,
                    settings.GetAlterTtlSettings().has_value()

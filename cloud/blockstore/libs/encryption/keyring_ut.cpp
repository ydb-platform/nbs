...
{
    Y_UNIT_TEST_F(
        ShouldGetTheSameEncryptionKeyHashesFromFileAndKeyring,
        TFixture)
    {
        TString fileKeyHash;
        {
        ...
        TString keyringKeyHash;
        {
            ...
            auto keyringOrError =
                mutableStorage->AddEndpoint(keyName, DefaultEncryptionKey);
    ...
    Y_UNIT_TEST_F(
        ShouldFailEncryptorCreationIfKeyringKeyLengthIsInvalid,
        TFixture)
    {
        const TString guid = CreateGuidAsString();
        const TString nbsDesc = "nbs_" + guid;
        ...
        auto keyringOrError =
            mutableStorage->AddEndpoint(keyName, "key_with_invalid_length");
        UNIT_ASSERT_C(!HasError(keyringOrError), keyringOrError.GetError());

...
{
    // Verify that the initializer exposes the default YAML mode before loading
    // ServerConfig and captures the local value in InitServerConfig.
    Y_UNIT_TEST(ShouldExposeDynamicYamlConfigurationFlag)
    {
        ...
        TOFStream(serverPath.GetPath()).Write(
            "ServerConfig { DynamicYamlConfigurationEnabled: true }");
        auto options = CreateOptions();
        options->ServerConfig = serverPath.GetPath();
        auto ci = TConfigInitializerYdb(std::move(options));

        // Expose the default before the local configuration is loaded.
        UNIT_ASSERT_VALUES_EQUAL(
            defaultConfig.GetDynamicYamlConfigurationEnabled(),
            ci.GetDynamicYamlConfigurationStaticallyEnabled());

        // Capture the flag from the local ServerConfig.
        ci.InitServerConfig();
        UNIT_ASSERT(ci.GetDynamicYamlConfigurationStaticallyEnabled());
    }

    // Verify that CMS changes the current ServerConfig without changing the
    // selected YAML mode, saved static snapshot or ICB controls.
    Y_UNIT_TEST(ShouldKeepStaticSnapshotAndSharedControls)
    {
        for (const bool staticValue: {false, true}) {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                !staticValue,
                ci.ServerConfig->GetDynamicYamlConfigurationEnabled());
            ...
            // the selected mode, static configuration or controls.
    ...
    // Verify that repeated CMS updates apply NamedConfigs and allowed feature
    // flags in PROTO mode even after CMS enables the ServerConfig YAML flag.
    Y_UNIT_TEST(ShouldPreserveProtoModeAcrossCmsUpdates)
    {
        // Start in PROTO mode with a known storage setting and feature flag.
        auto ci = TConfigInitializerYdb(CreateOptions());
        ci.InitKikimrConfig();
        InitStaticConfigs(ci);
        const auto initialThreshold = ci.StorageConfig->GetWriteBlobThreshold();
        ci.KikimrConfig->MutableFeatureFlags()
            ->SetEnableNodeBrokerDeltaProtocol(false);

        // Change the current ServerConfig flag through the first CMS update.
        NKikimrConfig::TAppConfig firstCms;
        auto* server = firstCms.AddNamedConfigs();
        server->SetName("Cloud.NBS.ServerAppConfig");
        server->SetConfig(
            "ServerConfig { DynamicYamlConfigurationEnabled: true }");
        ci.ApplyCMSConfigs(firstCms);
        UNIT_ASSERT(ci.ServerConfig->GetDynamicYamlConfigurationEnabled());

        // Apply another CMS update through the same entry point.
        NKikimrConfig::TAppConfig secondCms;
        auto* storage = secondCms.AddNamedConfigs();
        storage->SetName("Cloud.NBS.StorageServiceConfig");
        storage->SetConfig(TStringBuilder()
            << "WriteBlobThreshold: " << initialThreshold + 1);
        secondCms.MutableFeatureFlags()->SetEnableNodeBrokerDeltaProtocol(true);
        ci.ApplyCMSConfigs(secondCms);

        // Preserve PROTO mode while applying both parts of the second update.
        UNIT_ASSERT_VALUES_EQUAL(
            initialThreshold + 1,
            ci.StorageConfig->GetWriteBlobThreshold());
        UNIT_ASSERT(ci.KikimrConfig->GetFeatureFlags()
            .GetEnableNodeBrokerDeltaProtocol());
        UNIT_ASSERT(!ci.GetDynamicYamlConfigurationStaticallyEnabled());
        UNIT_ASSERT(ci.ServerConfig->GetDynamicYamlConfigurationEnabled());
    }
    ...
    Y_UNIT_TEST(ShouldBuildAllBlockstoreConfigs)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL(
            config.GetDescriptor()->field_count(),
            fields.size());
    }

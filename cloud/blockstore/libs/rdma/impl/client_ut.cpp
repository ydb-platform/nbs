...
Y_UNIT_TEST_SUITE(TRdmaClientTest){Y_UNIT_TEST(ShouldStartEndpoint){
...
auto logging = CreateLoggingService("console", TLogSettings{TLOG_RESOURCES});
...
Y_UNIT_TEST(ShouldHandleGetAddressInfoError)
{
    ...
    auto client = CreateClient(verbs, logging, monitoring, config);
...
Y_UNIT_TEST(ShouldProcessRequests)
{
    ...
    auto retryDelay = DurationToCyclesSafe(clientConfig->MaxResponseDelay) + 1;
...
Y_UNIT_TEST(ShouldHandleErrors)
{
    ...
    auto endpoint =
        client->StartEndpoint("::", 10020).GetValue(TDuration::Seconds(5));

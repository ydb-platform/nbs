...
Y_UNIT_TEST_SUITE(TRdmaClientTest)
{
    ...
    Y_UNIT_TEST(ShouldReuseChunks)
    {
        ...
        auto monitoring = CreateMonitoringServiceStub();
        auto clientConfig = std::make_shared<TClientConfig>();
        clientConfig->BufferPoolConfig.ChunkSize = 80_MB;
        clientConfig->BufferPoolConfig.MaxChunkAlloc = 4_MB;
        ...
        int maxRequestsInOneChunk = clientConfig->BufferPoolConfig.ChunkSize /
                                    clientConfig->BufferPoolConfig.MaxChunkAlloc;

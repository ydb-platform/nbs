...
// IResponseHandler interface is used to process the response on the user side.
...
// Connection status of IClientEndpoint and IClientSession
...
// IEndpointObserver interface is used handle status changes on the user side.
struct IEndpointHandler
{
    virtual ~IEndpointObserver() = default;

    virtual void UpdateStatus(EConnectionStatus status) = 0;
}

////////////////////////////////////////////////////////////////////////////////

// IClientSession is used to represent persistent multi-endpoint connection.
struct IClientSession
{
    virtual TResultOrError<IClientEndpointPtr> GetEndpoint(
        TString host,
        ui32 port) = 0;
}

...
{
    NProto::TNVMeDevice proto;
    if (!google::protobuf::util::JsonStringToMessage(jsonStr, &proto)
             .ok())
    {
        return MakeError(E_ARGUMENT, "Failed to parse input");
    }

    if (proto.GetSerialNumber().empty()) {
        return MakeError(E_ARGUMENT, "Empty serial number");
    }

    return proto.GetSerialNumber();
}
...
public:
    ...
    {
        LOG_INFO(ctx, TBlockStoreComponents::SERVICE, "List NVMe devices");
...
public:
    ...
    void DoBootstrap(const TActorContext& ctx) final
    {
        ...
        LOG_INFO(
            ctx,
            TBlockStoreComponents::SERVICE,
            "Release NVMe device %s",
            sn.Quote().c_str());

        auto request =
            std::make_unique<TEvDiskAgentPrivate::TEvReleaseNVMeDeviceRequest>();
        request->SerialNumber = sn;

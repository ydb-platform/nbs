...
struct TChangeNonreplDiskDevice: public NUnitTest::TBaseFixture
{
...
struct TChangeMirrorDiskDevice: public NUnitTest::TBaseFixture
{
    TTestExecutor Executor;
    TVector<NProto::TAgentConfig> Agents;
    std::unique_ptr<TDiskRegistryState> State;
...
Y_UNIT_TEST_SUITE(TDiskRegistryStateUpdatesTest)
{
    Y_UNIT_TEST(ShouldUpdateDiskBlockSize)
    {
        ...
        executor.WriteTx(
            ...
            {
                auto error =
                    state.UpdateDiskBlockSize(Now(), db, "", 1_KB, false);
                UNIT_ASSERT_VALUES_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                    "should reject a request with empty disk id");

                error = state.UpdateDiskBlockSize(
                    Now(),
                    db,
                    "nonexistent",
                    1_KB,
                ...
                error =
                    state.UpdateDiskBlockSize(Now(), db, diskId, 1_KB, false);
                UNIT_ASSERT_VALUES_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                ...
                error =
                    state.UpdateDiskBlockSize(Now(), db, diskId, 256_KB, false);
                UNIT_ASSERT_VALUES_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                ...
                error =
                    state.UpdateDiskBlockSize(Now(), db, diskId, 64_KB, false);
                UNIT_ASSERT_VALUES_EQUAL_C(
                    S_OK,
                    error.GetCode(),
                ...
                error =
                    state.UpdateDiskBlockSize(Now(), db, diskId, 1_KB, true);
                UNIT_ASSERT_VALUES_EQUAL_C(
                    S_OK,
                    error.GetCode(),
    ...
    Y_UNIT_TEST(ShouldUpdateDiskBlockSizeWhenDeviceResizesBadly)
    {
        ...
        executor.WriteTx(
            ...
            {
                auto error =
                    state.UpdateDiskBlockSize(Now(), db, diskId, 128_KB, false);
                UNIT_ASSERT_VALUES_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                    "should reject a request where device will be badly "
                    "resized");

                const auto blocksCountBeforeUpdate =
                    state.GetDevice(deviceId).GetBlocksCount();

                error =
                    state.UpdateDiskBlockSize(Now(), db, diskId, 128_KB, true);
                UNIT_ASSERT_VALUES_EQUAL_C(
                    S_OK,
                    error.GetCode(),
                    "should complete a request where device will be badly "
                    "resized "
                    "with force");
                UNIT_ASSERT_VALUES_UNEQUAL_C(
                    blocksCountBeforeUpdate,
                    state.GetDevice(deviceId).GetBlocksCount(),

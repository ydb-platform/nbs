...
Y_UNIT_TEST_SUITE(TDiskRegistryStateCreateTest)
{
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                auto error = state.CreateDiskFromDevices(
                    ...
                    nativeBlockSize,
                    NProto::STORAGE_MEDIA_SSD_NONREPLICATED,
                    {deviceByUUID("uuid-2.3"), deviceByUUID("uuid-1.3")},
                    &result);
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                auto error = state.CreateDiskFromDevices(
                    ...
                    nativeBlockSize,
                    NProto::STORAGE_MEDIA_SSD_LOCAL,
                    {deviceByUUID("uuid-1.4"), deviceByUUID("uuid-1.5")},
                    &result);
    ...
    Y_UNIT_TEST(ShouldAllocateDiskWithAdjustedBlockCount)
    {
        ...
        const ui64 testUnadjustedBlockCount =
            testDeviceSizeInBytes / DefaultLogicalBlockSize;
        const ui64 testBlockCount = 93_GB / DefaultLogicalBlockSize;

        const NProto::TAgentConfig agentConfig = AgentConfig(
            1,
            {Device(
                testDeviceName,
                testDeviceId,

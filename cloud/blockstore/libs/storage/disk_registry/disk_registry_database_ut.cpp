...
Y_UNIT_TEST_SUITE(TDiskRegistryDatabaseTest)
{
        ...
        executor.ReadTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    items[0].GetDevices(0).GetDeviceName(),
                    "dev-1");
            });
    }
    ...
    Y_UNIT_TEST(ShouldStoreKnownAgents)
    {
        ...
        executor.ReadTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(items[0].DevicesSize(), 1);
                UNIT_ASSERT_VALUES_EQUAL(
                    items[0].GetDevices(0).GetDeviceUUID(),
                    "id-1");
            });
    }
    ...
    Y_UNIT_TEST(ShouldStorePlacementGroups)
    {
        ...
        executor.ReadTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    items[0].GetDisks(0).DeviceRacksSize(),
                    2);
                UNIT_ASSERT_VALUES_EQUAL(
                    items[0].GetDisks(0).GetDeviceRacks(0),
                    "rack-1");
                UNIT_ASSERT_VALUES_EQUAL(
                    items[0].GetDisks(0).GetDeviceRacks(1),
                    "rack-2");
                UNIT_ASSERT_VALUES_EQUAL(
                    items[0].GetDisks(0).GetDiskId(),
                    "disk-1");
            });
    ...
    Y_UNIT_TEST(ShouldDeleteDisk)
    {
        ...
        executor.WriteTx([&](TDiskRegistryDatabase db) { db.DeleteDisk("2"); });
    ...
    Y_UNIT_TEST(ShouldStoreDiskState)
    {
        ...
        executor.ReadTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    "disk-0",
                    changes[0].State.GetDiskId());
                UNIT_ASSERT_VALUES_EQUAL(
                    "disk-2",
                    changes[1].State.GetDiskId());
            });
    }

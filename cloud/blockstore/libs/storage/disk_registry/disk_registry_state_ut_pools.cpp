...
Y_UNIT_TEST_SUITE(TDiskRegistryStatePoolsTest)
{
        ...
        auto allocate = [&] (auto db, ui32 deviceCount) {
            ...
            auto error = state.AllocateDisk(
                ...
                TDiskRegistryState::TAllocateDiskParams {
                    ...
                    .BlocksCount = deviceCount * DefaultDeviceSize /
    DefaultLogicalBlockSize, .MediaKind = NProto::STORAGE_MEDIA_SSD_LOCAL
                },
                &result);
        ...
        executor.WriteTx([&] (TDiskRegistryDatabase db) {
            ...
            UNIT_ASSERT_VALUES_EQUAL(E_BS_DISK_ALLOCATION_FAILED,
    error.GetCode());
        });
        ...
        executor.WriteTx([&] (TDiskRegistryDatabase db) {
            ...
            UNIT_ASSERT_VALUES_EQUAL("uuid-1.2",
    result.Devices[0].GetDeviceUUID()); UNIT_ASSERT_VALUES_EQUAL("uuid-2.1",
        ...
        const TVector agents{
            ...
            AgentConfig(
                ...
                {Device("dev-1", "uuid-2.1"),
                 Device("dev-2", "uuid-2.2") |
                     WithPool("local-ssd", NProto::DEVICE_POOL_KIND_LOCAL),
                 Device("dev-3", "uuid-2.3") | WithPool("bar"),
                 Device("dev-4", "uuid-2.4") | WithPool("baz")})};
    ...
    Y_UNIT_TEST(ShouldAdjustDeviceBlockCount)
    {
        ...
        const auto agent = AgentConfig(
            1,
            {Device("dev-1", "uuid-1") | WithPool("local1") |
                 WithTotalSize(2_GB + 16_KB, 512),
             Device("dev-2", "uuid-2") | WithTotalSize(1_GB + 16_KB),
             Device("dev-3", "uuid-3") | WithPool("local1") |
                 WithTotalSize(2_GB - 4_KB),
             Device("dev-4", "uuid-4") | WithTotalSize(1_GB - 8_KB, 8_KB),
             Device("dev-5", "uuid-5") | WithPool("local2") |
                 WithTotalSize(512_MB + 4_KB),
             Device("dev-6", "uuid-6") | WithPool("local2") |
                 WithTotalSize(512_MB),
             Device("dev-7", "uuid-7") | WithPool("global1") |
                 WithTotalSize(1100_MB),
             Device("dev-8", "uuid-8") | WithPool("global1") |
                 WithTotalSize(1010_MB),
             Device("dev-9", "uuid-9") | WithPool("global2") |
        ...
        UNIT_ASSERT_EQUAL(
            ...
            state.GetDevice("uuid-3").GetState());
        UNIT_ASSERT_EQUAL(
            NProto::DEVICE_STATE_ERROR,
            state.GetDevice("uuid-4").GetState());
        ...
        // check all local pools
        {
            ...
            SortBy(infos, [](auto& info) { return info.ChunkSize; });
    ...
    Y_UNIT_TEST(ShouldRejectAllocationForLocalPools)
    {
        ...
        auto allocate =
            [&](auto db, ui32 deviceCount, TVector<TString> agentIds)
        {
            TDiskRegistryState::TAllocateDiskResult result;
        ...
        executor.WriteTx(
            ...
            {
                auto [result, error] =
                    allocate(db, 2, {agents[0].GetAgentId()});
        ...
        executor.WriteTx(
            ...
            {
                auto [result, error] =
                    allocate(db, 2, {agents[1].GetAgentId()});
        ...
        executor.WriteTx(
            ...
            {
                auto [result, error] =
                    allocate(db, 3, {agents[1].GetAgentId()});
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                auto [result, error] = allocate(
                    db,
                    3,
                    {agents[0].GetAgentId(), agents[1].GetAgentId()});
    ...
    Y_UNIT_TEST(ShouldUpdateDevices)
    {
        ...
        const auto agentConfig = AgentConfig(
            1,
            {Device("dev-1", "uuid-1.1") |
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                const auto* agent =
                    state.FindAgent(newAgentConfig.GetAgentId());
                UNIT_ASSERT(agent);

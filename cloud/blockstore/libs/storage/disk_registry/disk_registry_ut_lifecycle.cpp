...
Y_UNIT_TEST_SUITE(TDiskRegistryTest)
{
    ...
    {
        const auto agentConfig = CreateAgentConfig(
            "agent-1",
            {Device("dev-1", "uuid-1", "rack-1", 10_GB),
             Device("dev-2", "uuid-2", "rack-1", 10_GB)});
    ...
    Y_UNIT_TEST(ShouldRetryCleanDevices)
    {
        ...
        auto runtime = TTestRuntimeBuilder().WithAgents({agentConfig}).Build();
    ...
    Y_UNIT_TEST(ShouldNotifyDisks)
    {
        ...
        const auto newAgent1 = CreateAgentConfig(
            "agent-1",
            {
                Device("dev-1", "uuid-1", "rack-1", 10_GB),
                Device("dev-2", "uuid-2", "rack-1", 10_GB),
    ...
    Y_UNIT_TEST(ShouldRejectDisconnectedAgent)
    {
        ...
        auto registerAgent = [&]
        {
            ...
            *request->Record.MutableAgentConfig() = agent;
            request->Record.MutableAgentConfig()->SetNodeId(
                runtime->GetNodeId(0));

            auto pipeEv =
                new IEventHandle(pipe, sender, request.release(), 0, 0);
    ...
    Y_UNIT_TEST(ShouldRejectDisconnectedAgentDynamically)
    {
        ...
        config.SetNonReplicatedAgentDisconnectRecoveryInterval(
            100'000);   // 100s
        config.SetNonReplicatedAgentTimeoutGrowthFactor(2);

        auto runtime =
            TTestRuntimeBuilder().WithAgents(agents).With(config).Build();
        // cooldown logic in TAgentList doesn't work with Now() <
        // cooldownTimeout see TAgentList::OnAgentDisconnected
    ...
    Y_UNIT_TEST(ShouldPublishDiskState)
    {
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL("vol0", item.DiskId);
            UNIT_ASSERT_EQUAL(NProto::DISK_STATE_ERROR, item.State);
    ...
    Y_UNIT_TEST(ShouldCountPublishDiskStateErrors)
    {
        ...
        auto logbrokerService =
            std::make_shared<TTestLogbrokerService>(MakeError(E_FAIL, "Test"));
        ...
        auto publishDiskStateError = counters->GetCounter(
            "AppCriticalEvents/PublishDiskStateError",
            true);
    ...
    Y_UNIT_TEST(ShouldRegisterUnavailableAgentAfterTimeout)
    {
        ...
        auto runtime = TTestRuntimeBuilder().With(defaultConfig).Build();
    ...
    Y_UNIT_TEST(ShouldSwitchToReadOnlyOnAgentFailure)
    {
        ...
        auto disconnectServer = [&]
        {
            auto event = new TEvTabletPipe::TEvServerDisconnected(
                ...
                *serverId);
            runtime->Send(
                new IEventHandle(*recipient, *sender, event),
                0,
                true);
        };
    ...
    Y_UNIT_TEST(ShouldDisableIOForBrokenDevices)
    {
        auto agentConfig = CreateAgentConfig(
            ...
            {
                CreateDeviceConfig(
                    {.Name = "NVMENBS01", .Id = "uuid-1", .SerialNum = "W"}),
                CreateDeviceConfig(
                    {.Name = "NVMENBS02", .Id = "uuid-2", .SerialNum = "X"}),
                CreateDeviceConfig(
                    {.Name = "NVMENBS03", .Id = "uuid-3", .SerialNum = "Y"}),
                CreateDeviceConfig(
                    {.Name = "NVMENBS04", .Id = "uuid-4", .SerialNum = "Z"}),
            });
        ...
        runtime->SetEventFilter(
            [&](auto&, TAutoPtr<IEventHandle>& event)
            {
                    ...
                    TEvDiskRegistry::EvRegisterAgentResponse)
                {
                    ...
                    auto& msg =
                        *event
                             ->Get<TEvDiskRegistry::TEvRegisterAgentResponse>();
            ...
            TTestRuntimeBuilder()
                ...
                .With(
                    []
                    {
                        ...
                        config.SetNonReplicatedAgentMaxTimeout(50'000);   // 50s
                        config.SetDiskRegistryInitialAgentRejectionThreshold(
                            50);   // 50%

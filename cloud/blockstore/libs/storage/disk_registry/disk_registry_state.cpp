    ...
    TInstant timestamp) -> TResultOrError<TAgentRegistrationResult>
{
    ...
    try {
        ...
        for (const auto& d: agent.GetDevices()) {
            ...
            const auto diskId = DeviceList.FindDiskId(uuid);
            ...
            if (IsActiveMigrationTarget(disk, uuid)) {
                TryRestartDeviceMigration(timestamp, db, diskId, disk, uuid);
                continue;
            }

            if (IsDeviceRetainedForMigrationAck(disk, uuid)) {
                continue;
            }
...
void TDiskRegistryState::StartDeviceMigrationOnTarget(
    TInstant now,
    TDiskRegistryDatabase& db,
    const TDiskId& sourceDiskId,
...
void TDiskRegistryState::StartDeviceMigrations(
    TInstant now,
    TDiskRegistryDatabase& db,
    const TVector<TDiskMigrations>& migrations,
    const TStartDeviceMigrationHandler& handler)
{
    THashSet<TString> affectedGroups;
    for (const auto& [sourceDiskId, deviceIds]: migrations) {
        bool started = false;
        for (const auto& deviceId: deviceIds) {
            auto result = StartDeviceMigration(now, db, sourceDiskId, deviceId);
            if (!HasError(result)) {
                started = true;
                // Subsequent allocations must see newly occupied racks, even
                // though the group is persisted only after the entire batch.
                if (auto* group = UpdatePlacementGroupInMemory(
                        sourceDiskId,
                        Disks.at(sourceDiskId),
                        "StartDeviceMigration"))
                {
                    affectedGroups.insert(group->GetGroupId());
                }
            }
            handler(sourceDiskId, deviceId, result);
        }

        if (started) {
            db.UpdateDisk(BuildDiskConfig(sourceDiskId, Disks.at(sourceDiskId)));
            AddReallocateRequest(db, sourceDiskId);
        }
    }
    ...
    const TDeviceId& targetDeviceId)
{
    try {
        ...
        NProto::TDeviceConfig targetDevice = GetDevice(targetDeviceId);
        if (targetDevice.GetDeviceUUID().empty()) {
            return MakeError(E_NOT_FOUND, TStringBuilder()
        ...
        UpdatePlacementGroup(
            ...
            Disks.at(sourceDiskId),
            "StartForceMigration");
        UpdateAndReallocateDisk(db, sourceDiskId, Disks.at(sourceDiskId));
        return targetDevice;
    } catch (const TServiceError& e) {
        return MakeError(e.GetCode(), e.what());
    }
    ...
    const TDiskState& diskState) const
{
    ...
    for (const auto& id: ReplicaTable.GetDeviceReplacements(config.GetDiskId()))
    {
    ...
    TVector<TDiskId>& affectedDisks)
{
    ...
    for (const auto& d: agent.GetDevices()) {
        ...
        if (IsActiveMigrationTarget(disk, deviceId)) {
            TryRestartDeviceMigration(timestamp, db, diskId, disk, deviceId);
            continue;
        }

        if (IsDeviceRetainedForMigrationAck(disk, deviceId)) {
            continue;
        }
    ...
    TDuration& timeout)
{
    ...
    if (newState == NProto::AGENT_STATE_WARNING) {
        DetachPathsOnAgentIfNeeded(db, *agent);
    } else if (newState == NProto::AGENT_STATE_ONLINE) {
        AttachPathsOnAgentIfNeeded(db, *agent);
    }
    ...
    bool dryRun) -> TUpdateCmsDeviceStateResult
{
    ...
    AttachPathIfNeeded(db, agent, path);
    ...
    bool dryRun) -> TUpdateCmsDeviceStateResult
{
    ...
    if (!dryRun && !HasError(result.Error) &&
        newState == NProto::DEVICE_STATE_WARNING)
    {
        DetachPathIfNeeded(db, *agent, path);
    } else if (
        !dryRun && !HasError(result.Error) &&
...
bool TDiskRegistryState::IsDeviceRetainedForMigrationAck(
    const TDiskState& disk,
...
bool TDiskRegistryState::IsActiveMigrationTarget(
    const TDiskState& disk,
    const TDeviceId& deviceId) const
{
    return disk.MigrationTarget2Source.contains(deviceId);
}
    ...
    TVector<NProto::TLaggingDevice> outdatedDevices)
{
    ...
    for (const auto& outdatedDevice: outdatedDevices) {
        ...
        if (isMigrationSource) {
            ...
            IsActiveMigrationTarget(*replicaState, outdatedDeviceUUID))
        {
            ...
            !IsDeviceRetainedForMigrationAck(
                *replicaState,
                outdatedDeviceUUID))
        {
...
TVector<TDiskRegistryState::TDiskMigrations>
TDiskRegistryState::BuildMigrationList() const
{
    ...
    TVector<TDiskMigrations> result;
    ...
    for (const auto& m: Migrations) {
        ...
        if (result.empty() || result.back().DiskId != m.DiskId) {
            result.push_back({m.DiskId, {}});
        }
        result.back().SourceDeviceIds.push_back(m.SourceDeviceId);
        ++migrationCount;
    }
...
void TDiskRegistryState::DetachPathIfNeeded(
    ...
    const TString& path)
{
    AttachDetachPathIfNeeded(db, agent, path, false);   // detach
}
...
void TDiskRegistryState::DetachPathsOnAgentIfNeeded(
    ...
    NProto::TAgentConfig& agent)
{
    AttachDetachPathsOnAgentIfNeeded(db, agent, false);   // detach
}
    ...
    NProto::EPathAttachState state)
{
    ...
    if (!StateTransitionAllowed(state, oldState)) {
        return MakeError(
            E_ARGUMENT,
            Sprintf(
...
void TDiskRegistryState::AttachPathIfNeeded(
    ...
    const TString& path)
{
    AttachDetachPathIfNeeded(db, agent, path, true);   // attach
}

void TDiskRegistryState::AttachPathsOnAgentIfNeeded(
    TDiskRegistryDatabase& db,
    NProto::TAgentConfig& agent)
{
    AttachDetachPathsOnAgentIfNeeded(db, agent, true);   // attach
}

void TDiskRegistryState::AttachDetachPathsOnAgentIfNeeded(
    TDiskRegistryDatabase& db,
    NProto::TAgentConfig& agent,
    bool attach)
{
    ...
    for (auto& path: paths) {
        AttachDetachPathIfNeeded(db, agent, path, attach);
    }
}
    ...
    bool attach)
{
    ...
    if (!inserted && actualState != desirableState &&
        actualState != companionState)
    {

    ...
    try {
        ...
        for (const auto& d: agent.GetDevices()) {
            ...
            if (IsMigrationTarget(disk, uuid)) {
                TryToRescheduleMigrationByTarget(
                    timestamp,
                    db,
                    diskId,
                    disk,
    ...
    TDiskId& affectedDisk)
{
    ...
    if (IsMigrationTarget(*disk, uuid)) {
        TryToRescheduleMigrationByTarget(now, db, diskId, *disk, uuid);
        return;
    }
...
bool TDiskRegistryState::IsDeviceInCurrentReplicaSet(
    const TDiskState& disk,
    const TString& deviceId) const
{
    Y_UNUSED(deviceId);

    return !disk.MasterDiskId.empty()
        // && FindPtr(disk.Devices, deviceId) // TODO: tests
        ;
}

void TDiskRegistryState::TryToRescheduleMigrationByTarget(
    TInstant now,
    TDiskRegistryDatabase& db,
    const TDiskId& diskId,
    TDiskState& disk,
    ...
    TVector<NProto::TLaggingDevice> outdatedDevices)
{
    ...
    for (const auto& outdatedDevice: outdatedDevices) {
        ...
        if (isMigrationSource) {
        ...
        } else if (IsMigrationTarget(*replicaState, outdatedDeviceUUID)) {
            TryToRescheduleMigrationByTarget(
                now,
                db,
                replicaId,
            ...
            !IsDeviceRetainedForMigrationAck(*replicaState, outdatedDeviceUUID))
        {

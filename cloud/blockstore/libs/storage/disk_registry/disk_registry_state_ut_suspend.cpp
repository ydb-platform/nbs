...
Y_UNIT_TEST_SUITE(TDiskRegistryStateSuspendTest)
{
    Y_UNIT_TEST_F(ShouldSuspendDevices, TFixture)
    {
        ...
        auto allocateDisk =
            [&](auto db, auto* diskId, auto deviceCount, auto kind)
        {
        ...
        WriteTx(
            ...
            {
                for (auto* uuid:
                     {"uuid-2.1", "uuid-2.2", "uuid-2.3", "uuid-2.4"}) {
        ...
        WriteTx([&](auto db)
                { UNIT_ASSERT_VALUES_EQUAL(S_OK, allocNRD(db, "nrd1", 3)); });

...
void TSendCmsRequestActor::Bootstrap(const TActorContext& ctx)
{
    switch (ActionType) {
        ...
        case NProto::TAction::PURGE_HOST: {
            auto request =
                std::make_unique<TEvDiskRegistryPrivate::TEvPurgeHostCmsRequest>(
                    AgentID,
                    DryRun);
    ...
    EAlertLevel alertLevel)
{
    ...
    if (DeviceUUID) {
        redirect << "./app?action=dev&TabletID=" << TabletID
                 << "&DeviceUUID=" << DeviceUUID;
    } else {
        redirect << "./app?action=agent&TabletID=" << TabletID
                 << "&AgentID=" << AgentID;
    }
    ...
    const TVector<TString>& dependentDiskIds)
{
    ...
    if (!HasError(error)) {
        ...
        message << "failed to send CMS request " << actionName << " for ";
        if (DeviceUUID) {
            message << "device " << DeviceUUID.Quote();
        } else {
            message << "agent " << AgentID.Quote();
        }
    ...
    TRequestInfoPtr requestInfo)
{
    if (!Config->GetEnableToChangeStatesFromDiskRegistryMonpage()) {
        RejectHttpRequest(ctx, *requestInfo, "Can't send CMS request from monpage");
        return;
    }
    ...
    const auto actionType =
        static_cast<NProto::TAction_EType>(actionTypeValue);
    ...
    TRequestInfoPtr requestInfo)
{
    ...
    if (!device.GetDeviceUUID()) {
        RejectHttpRequest(ctx, *requestInfo, "Unknown device");
        return;
    }

    const auto agentId = State->GetAgentId(device.GetNodeId());
    if (!agentId) {
        RejectHttpRequest(ctx, *requestInfo, "No agent id found for device");
        return;
    }
    ...
    LOG_INFO(
        ctx,
        TBlockStoreComponents::DISK_REGISTRY,
        "Send CMS request %s for device[%s] on agent[%s] from monitoring page "
        "(dryRun=%s)",
        NProto::TAction_EType_Name(actionType).c_str(),

...
Y_UNIT_TEST_SUITE(TDiskRegistryStateMirroredDisksTest)
{
    Y_UNIT_TEST(ShouldAllocateDisks)
    {
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                "dev-1",
                diskInfo.Devices[0].GetDeviceName());
    ...
    Y_UNIT_TEST(ShouldCleanupAfterAllocationFailure)
    {
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    "dev-5",
                    replicas[0][1].GetDeviceName());
                UNIT_ASSERT_VALUES_EQUAL(
                    "dev-6",
                    replicas[0][2].GetDeviceName());
                ASSERT_VECTORS_EQUAL(TVector<TString>{}, deviceReplacementIds);
            });
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                "dev-2",
                diskInfo.Devices[1].GetDeviceName());
            UNIT_ASSERT_VALUES_EQUAL(
                "dev-3",
                diskInfo.Devices[2].GetDeviceName());
            UNIT_ASSERT_VALUES_EQUAL(0, diskInfo.Migrations.size());
    ...
    Y_UNIT_TEST(ShouldResizeDisk)
    {
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    "dev-13",
                    replicas[0][3].GetDeviceName());
                UNIT_ASSERT_VALUES_EQUAL(
                    "dev-14",
                    replicas[0][4].GetDeviceName());
                UNIT_ASSERT_VALUES_EQUAL(
                    "dev-15",
                    replicas[0][5].GetDeviceName());
                ASSERT_VECTORS_EQUAL(TVector<TString>{}, deviceReplacementIds);
            });
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                error = AllocateDisk(
                    ...
                    nd);
                UNIT_ASSERT_SUCCESS(error);
                error = state.AllocateDiskReplicas(
                    Now(),
                    db,
                    "not-mirrored-disk",
                    1);
                UNIT_ASSERT_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                    "should not let to allocate replicas for a not mirrored "
                    "disk");
                ...
                UNIT_ASSERT_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                    "should not let to allocate replicas for a not master "
                    "disk");
                ...
                UNIT_ASSERT_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                    "should not let to allocate a lot of replicas which is "
                    "above "
                    "the limit");
                ...
                for (size_t i = 0; i < prototypeReplicaDevices.size(); i++) {
                    UNIT_ASSERT_VALUES_EQUAL_C(
                        ...
                        "added replica device should have same block size as "
                        "the "
                        "prototype replica");
                    UNIT_ASSERT_VALUES_EQUAL_C(
                        prototypeReplicaDevices[i].GetBlocksCount(),
                        addedReplicaDevices[i].GetBlocksCount(),
                        "added replica device should have same blocks count as "
                        "the "
                        "prototype replica");
                }
            });
    ...
    Y_UNIT_TEST(ShouldDeallocateReplicas)
    {
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                UNIT_ASSERT_SUCCESS(error);
                error =
                    state.DeallocateDiskReplicas(db, "not-mirrored-disk", 1);
                UNIT_ASSERT_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                    "should not let "
                ...
                UNIT_ASSERT_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                    "should not let to deallocate replicas for a not master "
                    "disk");
    ...
    Y_UNIT_TEST(ShouldUpdateReplicaCount)
    {
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                UNIT_ASSERT_SUCCESS(error);
                error =
                    state.UpdateDiskReplicaCount(db, "not-mirrored-disk", 2);
                UNIT_ASSERT_EQUAL_C(
                    E_ARGUMENT,
                    error.GetCode(),
                    "should not let "
    ...
    Y_UNIT_TEST(ShouldReplaceBrokenDevicesUponDeviceFailure)
    {
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db) mutable
            {
                // mirrored disk replicas should not delay host/device
                // maintenance
                // FIXME NBS-4169
    ...
    Y_UNIT_TEST(
        ShouldRemoveReplacementDeviceIdsUponReplacementDeviceReplacement)
    {
        TTestExecutor executor;
        executor.WriteTx([&](TDiskRegistryDatabase db) { db.InitSchema(); });
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db) mutable
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    ...
                    NProto::EDiskState_Name(
                        state.GetDiskState(affectedDisks[0])));
            });
    ...
    Y_UNIT_TEST(ShouldUpdateDiskInDBAfterDeviceReplacement)
    {
        ...
        TVector<TDeviceConfig> replicaDevices;
        const auto error = state.GetDiskDevices("disk-1/0", replicaDevices);
        UNIT_ASSERT_VALUES_EQUAL_C(S_OK, error.GetCode(), error.GetMessage());
        UNIT_ASSERT_VALUES_EQUAL(3, replicaDevices.size());
        UNIT_ASSERT_VALUES_EQUAL("uuid-10", replicaDevices[0].GetDeviceUUID());
        UNIT_ASSERT_VALUES_EQUAL("uuid-2", replicaDevices[1].GetDeviceUUID());
        UNIT_ASSERT_VALUES_EQUAL("uuid-3", replicaDevices[2].GetDeviceUUID());
        ...
        TVector<NProto::TDiskConfig> diskConfigs;
        executor.WriteTx(
            [&](TDiskRegistryDatabase db) mutable
            {
                UNIT_ASSERT(db.ReadDisks(diskConfigs));
                UNIT_ASSERT_VALUES_EQUAL(4, diskConfigs.size());
                UNIT_ASSERT_VALUES_EQUAL("disk-1", diskConfigs[0].GetDiskId());
                UNIT_ASSERT_EQUAL(
                    NProto::DISK_STATE_ONLINE,
                    diskConfigs[0].GetState());
                UNIT_ASSERT_VALUES_EQUAL(2, diskConfigs[0].GetReplicaCount());
                UNIT_ASSERT_VALUES_EQUAL(
                    1,
                    diskConfigs[0].DeviceReplacementUUIDsSize());
                UNIT_ASSERT_VALUES_EQUAL(
                    "uuid-10",
                    diskConfigs[0].GetDeviceReplacementUUIDs(0));
            });
    }
    ...
    Y_UNIT_TEST(ShouldReturnAffectedDiskListOnAgentFailure)
    {
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db) mutable
            {
                ...
                // The state of 'disk-1/0' has been changed so 'disk-1/0' should
                // be in 'affectedDisks'
                ASSERT_VECTORS_EQUAL(
                    TVector<TString>{"disk-1/0"},
                    affectedDisks);
    ...
    void DoTestShouldWaitForReplicaMigrationUponCmsRemoveRequest(bool isAgent)
    {
        TTestExecutor executor;
        executor.WriteTx([&](TDiskRegistryDatabase db) { db.InitSchema(); });
    ...
    Y_UNIT_TEST(ShouldReportDeviceReplacementFailureUponAgentFailure)
    {
        ...
        // intentionally setting agent count to 3 to get
        // E_DISK_ALLOCATION_FAILED upon ReplaceDevice call
        auto statePtr = TDiskRegistryStateBuilder()
                            .WithKnownAgents({
                                agentConfig1,
                                agentConfig2,
    ...
    Y_UNIT_TEST(ShouldReportDeviceReplacementFailureUponDeviceFailure)
    {
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db) mutable
            {
                ...
                UNIT_ASSERT_SUCCESS(state.GetDiskInfo("disk-1/0", diskInfo));
                UNIT_ASSERT_VALUES_EQUAL(
                    NProto::EDiskState_Name(NProto::DISK_STATE_ERROR),
                    NProto::EDiskState_Name(diskInfo.State));
            });
    ...
    Y_UNIT_TEST(ShouldReplaceDeviceOnAgentRegistration)
    {
        ...
        executor.WriteTx(
            ...
            {
                UNIT_ASSERT_SUCCESS(
                    state.RegisterAgent(db, agents[0], changeStateTs)
                        .GetError());
            });

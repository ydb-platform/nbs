...
Y_UNIT_TEST_SUITE(TDiskRegistryTest)
{
    ...
    Y_UNIT_TEST(ShouldRejectConfigWithBadVersion)
    {
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(response->GetStatus(), E_ABORTED);
        }
                ...
                switch (event->GetTypeRewrite()) {
                    case TEvDiskRegistry::EvRegisterAgentRequest: {
                        ...
                        UNIT_ASSERT_VALUES_EQUAL(
                            msg.Record.GetAgentConfig().DevicesSize(),
                            1);

                        auto& disk =
                            msg.Record.GetAgentConfig().GetDevices()[0];
    ...
    Y_UNIT_TEST(ShouldUpdateAgentStats)
    {
        ...
        auto counters = runtime->GetAppData(0)
                            .Counters->GetSubgroup("counters", "blockstore")
                            ->GetSubgroup("component", "disk_registry")
                            ->GetSubgroup("host", "test");

        auto agentCounters = counters->GetSubgroup("agent", "agent-1");
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                100,
                device->GetCounter("ReadCount")->Val());
            UNIT_ASSERT_VALUES_EQUAL(
                10000,
                device->GetCounter("ReadBytes")->Val());
            UNIT_ASSERT_VALUES_EQUAL(
                200,
                device->GetCounter("WriteCount")->Val());
            UNIT_ASSERT_VALUES_EQUAL(
                3000000,
                device->GetCounter("WriteBytes")->Val());

            auto p100 =
                device->GetSubgroup("percentiles", "Time")->GetCounter("100");
    ...
    Y_UNIT_TEST(ShouldRejectDestructiveUpdateConfig)
    {
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                "disk-1",
                response->Record.GetAffectedDisks(0));
        }
    }
        ...
        auto validate = [](auto& backup, const TString& name)
        {
            ...
            SortBy(
                *config.MutableKnownAgents(),
                [](auto& x) { return x.GetAgentId(); });
            SortBy(
                *backup.MutableAgents(),
                [](auto& x) { return x.GetAgentId(); });
            SortBy(
                *backup.MutablePlacementGroups(),
                [](auto& x) { return x.GetGroupId(); });
            SortBy(
                *backup.MutableDisks(),
                [](auto& x) { return x.GetDiskId(); });
            ...
            UNIT_ASSERT_VALUES_EQUAL_C(
                "disk-1",
                pg[0].GetDisks(0).GetDiskId(),
                name);
            UNIT_ASSERT_VALUES_EQUAL_C(
                "disk-2",
                pg[0].GetDisks(1).GetDiskId(),
                name);
            UNIT_ASSERT_VALUES_EQUAL_C(
                "disk-3",
                pg[0].GetDisks(2).GetDiskId(),
                name);

            UNIT_ASSERT_VALUES_EQUAL_C(
                "disk-4",
                pg[1].GetDisks(0).GetDiskId(),
                name);
            ...
            UNIT_ASSERT_VALUES_EQUAL_C(
                "agent-1",
                knownAgents[0].GetAgentId(),
                name);
            UNIT_ASSERT_VALUES_EQUAL_C(
                "agent-2",
                knownAgents[1].GetAgentId(),
                name);
            UNIT_ASSERT_VALUES_EQUAL_C(
                "agent-3",
                knownAgents[2].GetAgentId(),
                name);
        };
    ...
    struct TScheduleBackup: public TCurrentTestCase
    {
    ...
    Y_UNIT_TEST_F(ShouldScheduleBackup, TScheduleBackup)
    {
        ...
        auto runtime = TTestRuntimeBuilder().With(storageConfig).Build();
        ...
        const int backupsFileCount = std::distance(
    ...
    Y_UNIT_TEST(ShouldErrorOnBackup)
    {
        ...
        auto publishDiskStateError = counters->GetCounter(
            "AppCriticalEvents/DiskRegistryBackupFailed",
            true);
            ...
            [&](auto& runtime, auto& event)
            {
                ...
                switch (event->GetTypeRewrite()) {
                    ...
                    case TEvDiskRegistryPrivate::
                        EvFinishVolumeConfigUpdateRequest:
                        finishCount++;
                        break;
                }
    ...
    Y_UNIT_TEST(ShouldUpdateKnownDevicesAfterConfig)
    {
        ...
        auto freeBytes = diskRegistryGroup->GetSubgroup("pool", "default")
        ...
        auto unknownDevices = diskRegistryGroup->GetCounter("UnknownDevices");
        ...
        runtime->SetObserverFunc(
            [&](TAutoPtr<IEventHandle>& event)
            {
                if (event->GetTypeRewrite() ==
                    TEvDiskAgent::EvSecureEraseDeviceResponse)
                {

...
Y_UNIT_TEST_SUITE(TDiskRegistryStateTest)
{
    ...
    Y_UNIT_TEST(ShouldTellIfCanAllocateLocalWithDirtyDevices)
    {
        // Misc
        auto makeLocalDevice = [](const auto* name, const auto* uuid)
        {
            return Device(name, uuid) |
                   ...
                   WithTotalSize(93_GB, 512);
        };

        TTestExecutor executor;
        executor.WriteTx([&](TDiskRegistryDatabase db) { db.InitSchema(); });

        // Initially: agent/w 1 clean and 1 dirty local devices
        auto agent1 = AgentConfig(
            1,
            {
                makeLocalDevice("NVMELOCAL01-1", "uuid-1.1"),
                makeLocalDevice("NVMELOCAL02-1", "uuid-1.2"),
            });
        auto agent2 = AgentConfig(
            2,
            {
                makeLocalDevice("NVMELOCAL01-2", "uuid-2.1"),
                makeLocalDevice("NVMELOCAL02-2", "uuid-2.2"),
            });
        ...
        executor.WriteTx(
            [&](TDiskRegistryDatabase db)
            {
                ...
                // Suspend 1 device from agent-1
                {
                    auto error = state.SuspendDevice(db, "uuid-1.2");
                    UNIT_ASSERT(!HasError(error));
                };

                // Now we can't allocate disk of 2 devices on agent-1. But
                ...
                // Allocate 1 device from agent-2
                {
                    TDiskRegistryState::TAllocateDiskResult result{};
                    const auto allocationParams = makeAllocationQuery(1);
                    auto error = state.AllocateDisk(
                        TInstant::Now(),
                        db,
                        allocationParams,
                ...
                // Now we can't allocate disk of 2 devices on agent-1 nor on
                // agent-2
                {
                    const auto allocationParams = makeAllocationQuery(2);
                    auto suitableAgentId =

...
Y_UNIT_TEST_SUITE(TPartitionTest)
{
    ...
    Y_UNIT_TEST(ShouldReportUnflushedFreshByteCounters)
    {
        NProto::TStorageServiceConfig config;
        config.SetFlushThreshold(4_MB);
        config.SetFreshChannelWriteRequestsEnabled(true);
        ...
        ui64 unflushedFreshBlobByteCount = Max<ui64>();
        ui64 unflushedFreshLogicalBlocksByteCount = Max<ui64>();
            ...
            [&](TAutoPtr<IEventHandle>& event)
            {
                    ...
                    TEvStatsService::EvVolumePartCounters)
                {
                    ...
                    unflushedFreshBlobByteCount =
                        counters.UnflushedFreshBlobByteCount.Value;
                    unflushedFreshLogicalBlocksByteCount =
                        counters.UnflushedFreshLogicalBlocksByteCount.Value;
                }
        ...
        updateCounters();
        UNIT_ASSERT_VALUES_EQUAL(0, unflushedFreshBlobByteCount);
        UNIT_ASSERT_VALUES_EQUAL(0, unflushedFreshLogicalBlocksByteCount);

        partition.WriteBlocks(TBlockRange32::MakeOneBlock(0), 1);
        updateCounters();
        UNIT_ASSERT_GT(unflushedFreshBlobByteCount, 0);
        UNIT_ASSERT_VALUES_EQUAL(
            DefaultBlockSize,
            unflushedFreshLogicalBlocksByteCount);
        const auto blobByteCountAfterWrite = unflushedFreshBlobByteCount;

        partition.ZeroBlocks(TBlockRange32::MakeOneBlock(1));
        updateCounters();
        UNIT_ASSERT_GT(unflushedFreshBlobByteCount, blobByteCountAfterWrite);
        UNIT_ASSERT_VALUES_EQUAL(
            2 * DefaultBlockSize,
            unflushedFreshLogicalBlocksByteCount);
        ...
        partition.ZeroBlocks(TBlockRange32::MakeOneBlock(0));
        updateCounters();
        // The previous write to the 0-th block becomes garbage and should not
        // be accounted in unflushedFreshLogicalBlocksByteCount.
        UNIT_ASSERT_VALUES_EQUAL(
            2 * DefaultBlockSize,
            unflushedFreshLogicalBlocksByteCount);
        ...
        partition.WriteBlocks(TBlockRange32::MakeOneBlock(1), 1);
        updateCounters();
        // The previous zero of the 1-st block becomes garbage and should not
        // be accounted in unflushedFreshLogicalBlocksByteCount.
        UNIT_ASSERT_VALUES_EQUAL(
            2 * DefaultBlockSize,
            unflushedFreshLogicalBlocksByteCount);

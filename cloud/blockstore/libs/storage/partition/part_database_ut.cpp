    ...
    Y_UNIT_TEST(ShouldCorrectlyWriteAndReadCompactionMap)
    {
        ...
        executor.WriteTx([&] (TPartitionDatabase db) {
            ...
            for (size_t i = 0; i < RangeCount; ++i) {
                db.WriteCompactionMap(i * RangeSize, i % 100 + 1, i % 1023 + 1);
            }
        });
        ...
        for (size_t i = 0; i < RangeCount; ++i) {
            UNIT_ASSERT_VALUES_EQUAL(i* RangeSize, compactionMap1[i].BlockIndex);
            UNIT_ASSERT_VALUES_EQUAL(i % 100 + 1, compactionMap1[i].Stat.BlobCount);
            UNIT_ASSERT_VALUES_EQUAL(i % 1023 + 1, compactionMap1[i].Stat.BlockCount);
        }
        ...
        for (ui32 i = 0; i < RangeCount; i += RangeCountPerRun) {
            ...
            executor.ReadTx(
                [&](TPartitionDatabase db)
                {
                    db.ReadCompactionMap(compactionMap2, i * RangeSize, RangeCountPerRun);
                });
            UNIT_ASSERT_VALUES_EQUAL(Min(RangeCountPerRun, RangeCount - i), compactionMap2.size() - mapSize);
        }

        UNIT_ASSERT_VALUES_EQUAL(compactionMap1.size(), compactionMap2.size());
        for (ui32 i = 0; i < RangeCount; ++i) {
            UNIT_ASSERT_VALUES_EQUAL(compactionMap1[i].BlockIndex, compactionMap2[i].BlockIndex);
            UNIT_ASSERT_VALUES_EQUAL(compactionMap1[i].Stat.BlockCount, compactionMap2[i].Stat.BlockCount);
            UNIT_ASSERT_VALUES_EQUAL(compactionMap1[i].Stat.BlobCount, compactionMap2[i].Stat.BlobCount);
        }
    }
}

...
bool MayHaveBlock(const TBlocksFilter& filter, ui32 blockIndex,
                  ui64 commitId = Max<ui64>())
{
    return filter.MayHaveBlocksInMixedIndex(
        TBlockRange32::WithLength(blockIndex, 1), commitId);
}

void InitializeRanges(TBlocksFilter& filter, TVector<ui32> rangeIndices,
                      ui64 commitId)
{
...
void AddBlocksToMixedIndex(TBlocksFilter& filter, TBlockRange32 range,
                           ui64 commitId)
{
...
Y_UNIT_TEST_SUITE(TBlocksFilterTest)
{
    Y_UNIT_TEST(ShouldBeConservativeUntilRangeIsInitialized)
    {
        TBlocksFilter filter(0, BlocksPerRange, BlockCount);
    ...
    Y_UNIT_TEST(ShouldOnlyAddBlocksVisibleAtRangeCommitId)
    {
        ...
        UNIT_ASSERT(!filter.BlocksAddedToMixedIndex(0, rangeCommitId - 1));
        UNIT_ASSERT(filter.BlocksAddedToMixedIndex(1, rangeCommitId));
        UNIT_ASSERT(filter.BlocksAddedToMixedIndex(2, rangeCommitId + 1));

        UNIT_ASSERT_VALUES_EQUAL(2, filter.GetBlocksCount(0));
    ...
    Y_UNIT_TEST(ShouldClearRangeWhenCompactionHasNoConcurrentWrites)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL(0, filter.GetBlocksCount(0));
    ...
    Y_UNIT_TEST(ShouldHandleQueuedCompactionsInCommitOrder)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL(1, filter.GetBlocksCount(0));
        UNIT_ASSERT(!MayHaveBlock(filter, 0));
        UNIT_ASSERT(!MayHaveBlock(filter, 1));
        UNIT_ASSERT(MayHaveBlock(filter, 2));
    ...
    Y_UNIT_TEST(ShouldUpdateOnlyTheCompactedRange)
    {
        ...
        filter.BlocksAddedToMixedIndex(blockInFirstRange, 2);
        filter.BlocksAddedToMixedIndex(blockInSecondRange, 2);

        filter.CompactionStarted({1}, 10);
        filter.BlocksAddedToMixedIndex(blockInSecondRange + 1, 10);
        filter.CompactionFinished();
    ...
    Y_UNIT_TEST(ShouldHandlePartialLastRange)
    {
        ...
        TBlocksFilter filter(0, BlocksPerRange, PartialBlockCount);

        InitializeRanges(filter, {1}, 10);
        AddBlocksToMixedIndex(
            filter, TBlockRange32::WithLength(PartialBlockCount - 2, 2), 10);

        UNIT_ASSERT(MayHaveBlock(filter, PartialBlockCount - 2));
        UNIT_ASSERT(MayHaveBlock(filter, PartialBlockCount - 1));
        UNIT_ASSERT_VALUES_EQUAL(2, filter.GetBlocksCount(1));
        ...
        UNIT_ASSERT(!MayHaveBlock(filter, PartialBlockCount - 2));
        UNIT_ASSERT(!MayHaveBlock(filter, PartialBlockCount - 1));
        UNIT_ASSERT_VALUES_EQUAL(0, filter.GetBlocksCount(1));
    }
}

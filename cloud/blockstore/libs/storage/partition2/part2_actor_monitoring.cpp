    ...
    ui64 hiveTabletId)
{
    HTML (out) {
        DIV () {
            out << "<a href='app?TabletID=" << hiveTabletId << "&page=Groups"
                << "&tablet_id=" << storage.TabletID << "'>Channel history</a>";
        }
                        ...
                        if (auto latestEntry = channel.LatestEntry()) {
                            ...
                            if (cps.size()) {
                                    ...
                                    static_cast<ui32>(cps.size()))
                                {
                                    ...
                                    TABLED () {
                                        out << "PoolKind: " << poolKind;
                                    }
                                    TABLED () {
                                        out << "DataKind: " << channelKind;
                                    }
                                } else {
                                    // we need to output 2 cells, otherwise
                                    // table markup will be a bit broken
                                    TABLED () {
                                        out << "Ghost";
                                    }
                                    TABLED () {
        ...
        TABLE_SORTABLE()
        {
            ...
            TABLEBODY()
            {
                for (const auto& item: items) {
                    TABLER () {
                        TABLED () {
                            out << item.GetCheckpointId();
                        }
                        TABLED () {
                            out << item.GetCommitId();
                        }
                        TABLED () {
                            out << item.GetIdempotenceId();
                        }
                        TABLED () {
                            auto ts =
                                TInstant::MicroSeconds(item.GetDateCreated());
        ...
        TABLE_SORTABLE()
        {
            ...
            TABLEBODY()
            {
                for (const auto& kv: items) {
                    TABLER () {
                        TABLED () {
                            DumpCommitId(out, kv.first.CommitId());
                        }
                        TABLED_CLASS("view")
                        {
                            DumpBlobId(out, storage, kv.first);
                        }
                        TABLED () {
                            out << kv.second;
                        }
...
void DumpCleanupInfo(IOutputStream& out, const TForcedCleanupState& state)
{
    Y_UNUSED(state);

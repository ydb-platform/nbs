...
ui64 TPartitionThreadSafeState::StartFreshWrite(const NActors::TActorContext& ctx, ui64 blockCount)
{
    Y_UNUSED(ctx);
    TGuard guard(StateLock);
    ...
    bool isError)
{
    ...
    TVector<std::function<void(const NActors::TActorSystem* actorSystem)>> callbacks;

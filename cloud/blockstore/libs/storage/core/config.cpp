...
#define BLOCKSTORE_STORAGE_CONFIG_RW(xxx)                                      \
    ...
    xxx(FreshBlobByteCountFlushThreshold, ui32, 16_MB)                         \
                                                                               \
    xxx(SSDCompactionType, NProto::ECompactionType, NProto::CT_DEFAULT)        \
    xxx(HDDCompactionType, NProto::ECompactionType, NProto::CT_DEFAULT)        \
    ...
    xxx(MaxAcquireShadowDiskTotalTimeoutWhenNonBlocked,                        \
        TDuration,                                                             \
        Seconds(600))                                                          \
...
#define BLOCKSTORE_DURATION_FEATURES(xxx) xxx(MaxTimedOutDeviceStateDuration)
...
IOutputStream& operator<<(IOutputStream& out, NProto::ECompactionType ct)
{
    return out << ECompactionType_Name(ct);
}

IOutputStream& operator<<(IOutputStream& out, NProto::EVolumePreemptionType pt)
{
...
{
    os << "{ " << value.CertFile << ", " << value.CertPrivateKeyFile << " }";
}
...
TStorageConfig::TStorageConfig(
    ...
    NFeatures::TFeaturesConfigPtr featuresConfig)
    : Impl(
          new TImpl(std::move(storageServiceConfig), std::move(featuresConfig)))
{}
...
#define BLOCKSTORE_BINARY_FEATURE_GETTER(name)                           \
    bool TStorageConfig::Is##name##FeatureEnabled(                       \
        ...
        const {return Impl->FeaturesConfig                               \
                   ->IsFeatureEnabled(cloudId, folderId, diskId, #name); \
    }
...
#define BLOCKSTORE_STRING_FEATURE_GETTER(name)                          \
    TString TStorageConfig::Get##name##FeatureValue(                    \
        ...
        const TString& diskId)                                          \
        const {return Impl->FeaturesConfig                              \
                   ->GetFeatureValue(cloudId, folderId, diskId, #name); \
    }
    ...
    NCloud::NProto::EStorageMediaKind mediaKind)
{
    ...
    Y_ABORT_UNLESS(
        unit != 0);   // TODO: this check should be moved to nbs startup

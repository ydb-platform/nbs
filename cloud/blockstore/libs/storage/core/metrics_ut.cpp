...
using TBlobLoadMetrics = NCloud::NBlockStore::NStorage::NBlobMetrics::TBlobLoadMetrics;

IOutputStream& operator<<(
    IOutputStream& out,
    const TBlobLoadMetrics::TTabletMetric& rhs)
{
    out << "{" << rhs.ReadOperations.ByteCount << ","
               << rhs.ReadOperations.Iops << ","
...
IOutputStream& operator<<(
    IOutputStream& out,
    const std::pair<TBlobLoadMetrics::TChannel,
                    TBlobLoadMetrics::TGroupId>& rhs)
{
    out << "{" << rhs.first << ","
               << rhs.second << "}";
    return out;

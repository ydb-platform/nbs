...
Y_UNIT_TEST_SUITE(TMirrorPartitionLaggingDevicesTest)
{
    ...
    Y_UNIT_TEST(ShouldAddAndRemoveLaggingAgents)
    {
        ...
        runtime.SetRegistrationObserverFunc(
                ...
                const auto& actorId)
            {
                ...
                if (parentId == mirrorActorId) {
                    mirrorActorChildren.push_back(mirrorActorId);
                }
            });
        ...
        runtime.SetEventFilter(
            [&](TTestActorRuntimeBase&, TAutoPtr<IEventHandle>& event)
            {
                switch (event->GetTypeRewrite()) {
                    ...
                    // case TEvNonreplPartitionPrivate::EvMigrateNextRange: {
                    //     // Disable migrations.
                    //     return true;
                    // }
                }
        ...
        UNIT_ASSERT_VALUES_EQUAL(3, Count(readActors, env.ReplicaActors[0]));
        readActors.clear();

        env.ReadAndCheckContents(
            TBlockRange64::MakeOneBlock(firstAndSecondRows.Start - 1),
            'A');
        UNIT_ASSERT_VALUES_EQUAL(
            3,
            Count(readActors, env.ReplicaActors[0]) +
        ...
        env.ReadAndCheckContents(
            TBlockRange64::MakeOneBlock(firstAndSecondRows.End + 1),
            'A');
        UNIT_ASSERT_VALUES_EQUAL(
            3,
            Count(readActors, env.ReplicaActors[0]) +
        ...
        env.ReadAndCheckContents(firstRow, 'A');
        UNIT_ASSERT_VALUES_EQUAL(
            3,
            Count(readActors, env.ReplicaActors[0]) +
                Count(readActors, env.ReplicaActors[1]) +

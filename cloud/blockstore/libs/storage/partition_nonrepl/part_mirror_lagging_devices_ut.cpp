...
struct TTestEnv
{
    ...
    void AddLaggingAgent(const TString& agentId)
    {
        ...
        for (const auto& laggingDevice: laggingAgent.GetDevices()) {
            Cerr << "device: " << laggingDevice.GetDeviceUUID() << ":"
                 << laggingDevice.GetRowIndex() << Endl;
        }
        Cerr << "-------------------------------" << Endl;
    ...
    Y_UNIT_TEST(ShouldAddAndRemoveLaggingAgents)
    {
            ...
            [&](TTestActorRuntimeBase&, TAutoPtr<IEventHandle>& event)
            {
                switch (event->GetTypeRewrite()) {
                    ...
                    case TEvents::TEvPoisonPill::EventType: {
                        Cerr << "poison pill catcher; Recipient = "
                             << event->Recipient << Endl;
        ...
        runtime.SetScheduledEventFilter(
                ...
                auto&& deadline)
            {
                ...
                switch (event->GetTypeRewrite()) {
                    case TEvents::TEvPoisonPill::EventType: {
                        Cerr << "poison pill catcher 2; Recipient = "
                             << event->Recipient << Endl;
                        auto it = Find(mirrorActorChildren, event->Recipient);
                        if (it != mirrorActorChildren.end()) {
                            mirrorActorChildren.erase(it);
        ...
        Cerr << "replica2Proxy = " << replica2Proxy << Endl;
        ...
        Cerr << "replica1Proxy = " << replica1Proxy << Endl;
        unavailableAgents.clear();

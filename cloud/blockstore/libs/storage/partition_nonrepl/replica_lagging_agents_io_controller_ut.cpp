...
constexpr ui64 DeviceBlockCount = 8196;
...
struct TTestEnv
{
        ...
        ui32 replicaIndex)
    {
        ...
        for (ui32 i = 0; i < DeviceCountPerReplica; i++) {
            const ui32 nodeId =
                runtime.GetNodeId(replicaIndex * DeviceCountPerReplica + i);
            AddDevice(
                nodeId,
                DeviceBlockCount,
                Sprintf("uuid-%u", nodeId),
                Sprintf("agent-%u", nodeId),
                devices);
        }
...
Y_UNIT_TEST_SUITE(TReplicaLaggingAgentsIOControllerActorTest)
{
        ...
        runtime.SetEventFilter(
            [&](TTestActorRuntimeBase&, TAutoPtr<IEventHandle>& event)
            {
                switch (event->GetTypeRewrite()) {
                    ...
                    case TEvVolumePrivate::EvSmartMigrationFinished: {

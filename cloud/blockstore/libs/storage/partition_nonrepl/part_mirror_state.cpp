    ...
    , Migrations(std::move(migrations))
{
    ...
    Cerr << "Replica configs:" << Endl;
    for (auto& replicaInfo: ReplicaInfos) {
        for (const auto& device : replicaInfo.Config->GetDevices()) {
...
ui32 TMirrorPartitionState::GetReplicaIndex(NActors::TActorId actorId) const
{
    auto it = FindIf(
        ...
        {
            return replicaActors.LaggingProxyActor == actorId ||
                   replicaActors.PartActor == actorId;
        });
    ...
    NActors::TActorId* actorId)
{
    ...
    for (ui32 i = 0; i < ReplicaActors.size(); ++i) {
        ...
        if (DevicesReadyForReading(replicaIndex, readRange)) {
            *actorId = GetReplicaActors()[replicaIndex];
            return {};
        }
    }
...
void TMirrorPartitionState::ResetLaggingReplicaProxy(ui32 replicaIndex)
{
    ...
    ReplicaActors[replicaIndex].LaggingProxyActor = ReplicaActors[replicaIndex].PartActor;
}
    ...
    const NActors::TActorId& actorId)
{
    Y_DEBUG_ABORT_UNLESS(ReplicaActors.size() > replicaIndex);
    // Y_DEBUG_ABORT_UNLESS(ReplicaActors.size() == LaggingReplicaProxies.size());
    ReplicaActors[replicaIndex].LaggingProxyActor = actorId;
...
bool TMirrorPartitionState::IsLaggingProxySet(ui32 replicaIndex) const
{
    ...
    // Y_DEBUG_ABORT_UNLESS(ReplicaActors.size() ==
    // LaggingReplicaProxies.size());
    return ReplicaActors[replicaIndex].PartActor !=
           ReplicaActors[replicaIndex].LaggingProxyActor;
}

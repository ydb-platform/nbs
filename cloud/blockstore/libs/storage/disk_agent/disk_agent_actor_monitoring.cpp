        ...
        TABLE_SORTABLE_CLASS("table table-bordered")
        {
            ...
            for (const auto& config: State->GetDevices()) {
                ...
                TABLER () {
                    ...
                    TABLED () {
                        const auto bytes =
                            config.GetBlockSize() * config.GetBlocksCount();
                        out << config.GetBlocksCount() << " ("
                            << FormatByteSize(bytes) << ")";
                    }
                    TABLED () {
                        out << config.GetPoolName();
                    }
                    TABLED () {
                        out << config.GetTransportId();
                    }
                    TABLED () {

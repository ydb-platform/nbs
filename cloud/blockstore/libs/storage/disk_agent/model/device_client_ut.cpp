...
Y_UNIT_TEST_SUITE(TDeviceClientTest)
{
    Y_UNIT_TEST_F(TestAcquireReleaseAccess, TFixture)
    {
        ...
        auto error =
            AcquireDevices(client, TAcquireParamsBuilder().SetUuids({"uuid1"}));
        UNIT_ASSERT_VALUES_EQUAL(E_ARGUMENT, error.GetCode());

        error = AcquireDevices(
            client,
            TAcquireParamsBuilder().SetUuids({"uuid3"}).SetClientId("client"));
        UNIT_ASSERT_VALUES_EQUAL(E_NOT_FOUND, error.GetCode());

        error = ReleaseDevices(
            client,
            TReleaseParamsBuilder().SetUuids({"uuid3"}).SetClientId("client"));
        ...
        error = AcquireDevices(
            client,
            TAcquireParamsBuilder().SetUuids({"uuid2"}).SetClientId("client"));
        UNIT_ASSERT_VALUES_EQUAL(S_OK, error.GetCode());
        ...
        error = AcquireDevices(
            client,
            TAcquireParamsBuilder().SetUuids({"uuid2"}).SetClientId(
                "another_client"));
    ...
    Y_UNIT_TEST_F(TestAcquireAtomicity, TFixture)
    {
        ...
        error = ReleaseDevices(
            client,
            TReleaseParamsBuilder().SetUuids({"uuid1"}).SetClientId(
                "another_client"));
        UNIT_ASSERT_VALUES_EQUAL(S_OK, error.GetCode());

        error = ReleaseDevices(
            client,
            TReleaseParamsBuilder().SetUuids({"uuid2"}).SetClientId("client"));
        UNIT_ASSERT_VALUES_EQUAL(S_OK, error.GetCode());

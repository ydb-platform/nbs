...
Y_UNIT_TEST_SUITE(TVolumeDatabaseTest)
{
    ...
    Y_UNIT_TEST(ShouldStoreCheckpointShadowDiskInfo)
    {
        ...
        executor.WriteTx(
            [&](TVolumeDatabase db)
            { db.UpdateShadowDiskState(1, 512, EShadowDiskState::Preparing); });
        ...
        executor.WriteTx(
            [&](TVolumeDatabase db)
            { db.UpdateShadowDiskState(1, 1024, EShadowDiskState::Ready); });
        ...
        executor.ReadTx(
            [&](TVolumeDatabase db)
            {
                ...
                for (ui32 i = 0; i < 2; ++i) {
                    ...
                    UNIT_ASSERT_VALUES_EQUAL(
                        8 * i,
                        stat.GetCompactionGarbageScore());
                    UNIT_ASSERT_VALUES_EQUAL(
                        9 * i,
                        stat.GetCleanupQueueBytes());
                    UNIT_ASSERT_VALUES_EQUAL(
                        10 * i,
                        stat.GetGarbageQueueBytes());
                    UNIT_ASSERT_VALUES_EQUAL(
                        11 * i,
                        stat.GetLogicalUsedBytesCount());
                }
            });
    }
    ...
    Y_UNIT_TEST(ShouldRemoveOutdatedDeletedCheckpointsFromHistory)
    {
        ...
        executor.ReadTx(
            [&](TVolumeDatabase db)
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    0,
                    outdatedCheckpointRequestIds.size());
    ...
    Y_UNIT_TEST(ShouldStoreVolumeHistory)
    {
        ...
        executor.WriteTx(
            [&](TVolumeDatabase db)
            {
                ...
                // second
                db.WriteHistory(CreateRemoveClient(
                    TInstant::Seconds(2),
                    0,
                    one.GetClientId(),
                    "reason1",
        ...
        executor.ReadTx(
            [&](TVolumeDatabase db)
            {
                ...
                CheckRemoveClientLog(
                    two.GetClientId(),
                    "reason2",
                    records.Items[0].Operation.GetRemove());
                CheckRemoveClientLog(
                    one.GetClientId(),
                    "reason1",
                    records.Items[1].Operation.GetRemove());
    ...
    Y_UNIT_TEST(ShouldReadOutdatedHistory)
    {
        ...
        executor.ReadTx(
            [&](TVolumeDatabase db)
            {
                ...
                UNIT_ASSERT(
                    db.ReadOutdatedHistory(TInstant::Seconds(1), 1, records));

                UNIT_ASSERT_VALUES_EQUAL(1, records.size());

                UNIT_ASSERT_VALUES_EQUAL(
                    THistoryLogKey(TInstant::Seconds(1)),
                    records[0]);
            });
    ...
    Y_UNIT_TEST(ShouldReadExactNumberOfRecords)
    {
        ...
        executor.WriteTx(
            [&](TVolumeDatabase db)
            {
                ...
                db.WriteHistory(
                    CreateAddClient(TInstant::Seconds(1), 0, one, {}, {}, {}));
                db.WriteHistory(
                    CreateAddClient(TInstant::Seconds(1), 1, two, {}, {}, {}));
            });
        ...
        executor.ReadTx(
            [&](TVolumeDatabase db)
            {
                ...
                UNIT_ASSERT(
                    db.ReadHistory({TInstant::Seconds(2), 1}, {}, 2, records));

                UNIT_ASSERT_VALUES_EQUAL(2, records.Items.size());
                UNIT_ASSERT_C(
                    records.HasMoreItems(),
                    "Should not read all records");
        ...
        executor.ReadTx(
            [&](TVolumeDatabase db)
            {
                ...
                UNIT_ASSERT(
                    db.ReadHistory({TInstant::Seconds(2), 1}, {}, 0, records));
    ...
    Y_UNIT_TEST(ShouldUpdateVolumeParams)
    {
        ...
        const auto paramVecToMap =
            [](const TVector<TRuntimeVolumeParamsValue>& params)
        {
    ...
    Y_UNIT_TEST(ShouldStoreStorageConfig)
    {
        ...
        executor.ReadTx(
            [&](TVolumeDatabase db)
            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    100,
                    serviceConfig->GetCompactionRangeCountPerRun());
            });
    }
...
{
    out << '{' << "Key: " << value.Key << ", " << "Value: " << value.Value

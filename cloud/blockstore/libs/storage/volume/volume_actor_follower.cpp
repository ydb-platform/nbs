...
void TVolumeActor::DestroyOldLeaderIfNeeded(const NActors::TActorContext& ctx)
{
    for (const auto& leader: State->GetAllLeaders()) {
        ...
        LOG_INFO(
            ...
            DestroyLeaderInfo.TryCount,
            FormatDuration(DestroyLeaderInfo.DelayProvider.GetDelay()).c_str());
        ...
        ctx.Schedule(
            DestroyLeaderInfo.DelayProvider.GetDelay(),
    ...
    const NActors::TActorContext& ctx)
{
    ...
    if (HasError(msg->GetError())) {
        ...
        if (leader) {
            DestroyLeaderInfo.DelayProvider.IncreaseDelay();
            DestroyOldLeaderIfNeeded(ctx);
        }
        return;
    }
    ...
    const TActorContext& ctx)
{
    ...
    if (link.FollowerDiskId != State->GetDiskId()) {
        TString message =
            TStringBuilder()
            << "Wrong follower disk id: " << link.FollowerDiskId.Quote()
            << ", expected: " << State->GetDiskId().Quote();
        LOG_ERROR(
            ctx,
            TBlockStoreComponents::VOLUME,

...
Y_UNIT_TEST_SUITE(TDurableClientTest)
{
    ...
    Y_UNIT_TEST(ShouldUseNonRetriableListWhenSpecifiedAndNotRetryListed)
    {
        auto client = std::make_shared<TTestService>();
    ...
    Y_UNIT_TEST(ShouldUseNonRetriableListWhenSpecifiedAndNotRetrySessionError)
    {
        auto client = std::make_shared<TTestService>();
        ...
        client->PingHandler = [&](std::shared_ptr<NProto::TPingRequest> request)
        {
            ...
            if (++requestsCount < maxRequestsCount) {
                auto& error = *response.MutableError();
                error.SetCode(E_BS_INVALID_SESSION);
            }
        ...
        const auto& response = future.GetValue(TDuration::Seconds(5));
        UNIT_ASSERT_EQUAL(response.GetError().GetCode(), E_BS_INVALID_SESSION);

        UNIT_ASSERT_EQUAL(requestsCount, 1);
    }

    Y_UNIT_TEST(ShouldUseNonRetriableListWhenSpecifiedAndRetryNotListed)
    {
        auto client = std::make_shared<TTestService>();

...
struct TStorageOptions
{
    ...
    // Requested number of threads serving the endpoint. Clamped to
    // [1, Min(VhostQueuesCount, TServerConfig::ThreadsCount)] before the
    // endpoint is started, so a zero value means a single thread.
    ui32 ThreadCount = 0;

...
void DumpHistogram(const THistogramBase& hist)
{
    ...
    for (const auto& [p, name]: percentiles) {
        Cout << name << " : " << hist.GetValueAtPercentile(100 * p) << Endl;
    }
    Cout << Endl;
}
...
private:
    void ReadBlocks(TRequest* request)
    {
        ...
        auto future = Storage->ReadBlocks(callContext, std::move(req), sglist);
    ...
    void WriteBlocks(TRequest* request)
    {
        ...
        auto future = Storage->WriteBlocks(callContext, std::move(req), sglist);
    ...
    void ProcessResponse(TRequest* request)
    {
        ...
        Y_ABORT_UNLESS(
            !HasError(request->Response),
            "%s",
            FormatError(request->Response).c_str());

        if (request->WriteRequest) {
            ++Stats.NumWriteOps;
            Stats.BytesWritten += request->BlocksCount * Options->BlockSize;
            WriteLatency.RecordValue(
                (request->RecvTs - request->SendTs) / CyclesPerNanosecond);
        } else {
            ++Stats.NumReadOps;
            Stats.BytesRead += request->BlocksCount * Options->BlockSize;
            ReadLatency.RecordValue(
                (request->RecvTs - request->SendTs) / CyclesPerNanosecond);
        }
    }
};
...
private:
    ...
    void ReportProgress(TDuration elapsed)
    {
        ...
        Cout << "IOPS: "
             << round((stats.NumReadOps + stats.NumWriteOps) / seconds)
             << ", BW(r): " << FormatByteSize(stats.BytesRead / seconds) << "/s"
             << ", BW(w): " << FormatByteSize(stats.BytesWritten / seconds)
             << "/s" << ", BW(rw): "
             << FormatByteSize((stats.BytesRead + stats.BytesWritten) / seconds)
             << "/s" << Endl;
    ...
    void ReportFinalStats(TDuration elapsed)
    {
        ...
        Cout << "=== Total ===" << Endl << "Reads    : " << stats.NumReadOps
             << Endl << "Writes   : " << stats.NumWriteOps << Endl
             << "Bytes(r) : " << FormatByteSize(stats.BytesRead) << Endl
             << "Bytes(w) : " << FormatByteSize(stats.BytesWritten) << Endl
             << Endl;

        Cout << "=== Average ===" << Endl << "IOPS     : "
             << round((stats.NumReadOps + stats.NumWriteOps) / seconds) << Endl
             << "Lat(r)   : " << round(RequestSender->ReadLatency.GetMean())
             << " ns" << Endl
             << "Lat(w)   : " << round(RequestSender->WriteLatency.GetMean())
             << " ns" << Endl
             << "BW(r)    : " << FormatByteSize(stats.BytesRead / seconds)
             << "/s" << Endl
             << "BW(w)    : " << FormatByteSize(stats.BytesWritten / seconds)
             << "/s" << Endl << "BW(rw)   : "
             << FormatByteSize((stats.BytesRead + stats.BytesWritten) / seconds)

#include "aligned_direct_io_block_test_scenario.h"
...
class TAlignedDirectIoBlockScenario: public ITestScenario
{
private:
...
private:
    ...
    {
        TAlignedDirectIoBlockScenario* Scenario;
        ui32 Index;

        TThread(TAlignedDirectIoBlockScenario* testExecutor, ui32 index)
...
void TAlignedDirectIoBlockScenario::DoRequest(
    ui16 rangeIdx,
    double secondsSinceTestStart,
    IContext* context)
{
...
void TAlignedDirectIoBlockScenario::OnResponse(
    TInstant startTs,
    ui16 rangeIdx,
    TStringBuf reqType,
    IContext* context)
...
void TAlignedDirectIoBlockScenario::DoReadRequest(
    ui16 rangeIdx,
    IContext* context)
{
...
void TAlignedDirectIoBlockScenario::DoWriteRequest(
    ui16 rangeIdx,
    IContext* context)
{
    auto& range = Ranges[rangeIdx];
    ...
    const TLog& log)
{
    return ITestScenarioPtr(
        new TAlignedDirectIoBlockScenario(std::move(configHolder), log));
}

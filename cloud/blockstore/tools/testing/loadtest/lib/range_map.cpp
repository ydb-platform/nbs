...
TMaybe<TBlockRange64> TRangeMap::GetBlock(const TBlockRange64& range, bool exact)
{
    ...
    if (range.Overlaps(r)) {
        auto newRange = TBlockRange64(Max(range.Start, r.Start), Min(range.End, r.End));

        if (newRange.End != r.End) {
            Blocks.insert(TBlockRange64(newRange.End + 1, r.End));
        }
        ...
        if (range.Size() >= result.Size()) {
            ...
            auto newRange = TBlockRange64::WithLength(result.Start, Min(result.Size(), range.Size()));
            Blocks.insert(TBlockRange64(newRange.End + 1, result.End));
            return newRange;
        }
    }
}

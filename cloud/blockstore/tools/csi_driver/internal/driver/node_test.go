...
func doTestPublishUnpublishVolumeForKubevirtHelper(
	...
	if backend == "nbs" {
		...
		nbsClient.On("StartEndpoint", ctx, &nbs.TStartEndpointRequest{
			...
			VhostDiscardEnabled: true, // This is true because enableDiscard for newNodeService is true
...
func TestPublishUnpublishVolumeForKubevirtTable(t *testing.T) {
...
func TestStagedPublishUnpublishVolumeForKubevirtTable(t *testing.T) {
	deviceName1 := "test-disk-name-42"
	rqc32 := uint32(32)
...
func TestStagedPublishUnpublishDiskForKubevirtNegativeQueueCount(t *testing.T) {
	tempDir := t.TempDir()
...
func TestStagedPublishUnpublishDiskForKubevirtZeroQueueCount(t *testing.T) {
	tempDir := t.TempDir()
	...
	require.ErrorContains(t, err, "at least 1 request queue")
}

func TestStagedPublishUnpublishDiskForKubevirtInvalidQueueCount(t *testing.T) {
	tempDir := t.TempDir()

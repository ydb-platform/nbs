...
func (m *mounter) HasBlockDevice(ctx context.Context, device string) (bool, error) {
	...
	out, err := exec.CommandContext(ctxWithTimeout, "blockdev", "--getsize64", device).CombinedOutput()
	if err != nil {
...
func (m *mounter) IsFilesystemExisted(ctx context.Context, device string) (bool, error) {
	...
	out, err := exec.CommandContext(ctxWithTimeout, "blkid", device).CombinedOutput()

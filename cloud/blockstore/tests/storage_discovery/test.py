...
def test_config_comparison(
    ...
    crit = disk_agent.counters.find({'sensor': 'DiskAgentCriticalEvents/DiskAgentConfigMismatch'})
    ...
    if cmp == 'mismatch':
        ...
        crit = disk_agent.counters.find(
            {'sensor': 'DiskAgentCriticalEvents/DiskAgentConfigMismatch'})
        assert crit is not None
        assert crit['value'] == 2
...
@pytest.mark.parametrize("source", [EBackupDiskRegistryStateSource.BACKUP_DISK_REGISTRY_STATE_SOURCE_LOCAL_DB, EBackupDiskRegistryStateSource.BACKUP_DISK_REGISTRY_STATE_SOURCE_RAM])

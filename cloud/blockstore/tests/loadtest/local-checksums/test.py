...
TESTS = [
    TestCase(
        "mirror2",
        "cloud/blockstore/tests/loadtest/local-checksums/local-mirror2-basic.txt",
        agent_count=2,
        device_count=3,
    ),
    TestCase(
...
def __get_sensors(mon_port):
    ...
    read_requests = get_sensor(sensors, default_value=0, component='service', subcomponent='data_integrity', sensor="ReadRequests")
    write_requests = get_sensor(sensors, default_value=0, component='service', subcomponent='data_integrity', sensor="WriteRequests")
    read_checksum_mismatch = get_sensor(sensors, default_value=0, component='service', subcomponent='data_integrity', sensor="ReadChecksumMismatch")
    write_checksum_mismatch = get_sensor(sensors, default_value=0, component='service', subcomponent='data_integrity', sensor="WriteChecksumMismatch")
    return read_requests, write_requests, read_checksum_mismatch, write_checksum_mismatch
...
def __run_test(test_case):
    ...
    try:
        ...
        read_requests, write_requests, read_checksum_mismatch, write_checksum_mismatch = __get_sensors(nbs.mon_port)
        if read_requests > 0:
            raise Exception("hmmmm kek")

        # assert read_requests > 0, read_requests
        assert write_requests > 0, write_requests
        assert read_checksum_mismatch == 0, read_checksum_mismatch
        assert write_checksum_mismatch == 0, write_checksum_mismatch

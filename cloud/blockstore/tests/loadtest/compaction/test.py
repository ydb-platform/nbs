...
def data_count_by_cahnnels(nbs_client_binary_path, nbs_port):
...
def compaction_test(compaction_threshold_, mixed_blocks_, mixed_blobs_, merged_blocks_, merged_blobs_):
    ...
    storage.CompactionToMergedThreshold = compaction_threshold_
    ...
    for i in range(DEFAULT_BLOCK_COUNT):
        write_data += bytes(i % 256 for j in range(DEFAULT_BLOCK_SIZE))
    ...
    result_compaction = check_output([
        nbs_client_binary_path, "ExecuteAction",
        "--action", "compactrange",
        "--input-bytes", '{"DiskId":"vol0",'
                         '"StartIndex":"0",'
    ...
    operation_id = json.loads(result_compaction)["OperationId"]

    is_completed = False
    while not is_completed:
        status_compaction = check_output([
            nbs_client_binary_path, "ExecuteAction",
        ...
        status_compaction_json = json.loads(status_compaction)
        logging.info(f"status_compaction: {status_compaction}")
        is_completed = False if "IsCompleted" not in status_compaction_json \
            else status_compaction_json["IsCompleted"]
        if not is_completed:
            time.sleep(10)

    fresh_blocks, mixed_blocks, mixed_blobs, merged_blocks, merged_blobs = \
        data_count_by_cahnnels(nbs_client_binary_path, nbs.nbs_port)
    logging.info(f"fresh_blocks={fresh_blocks}")
    logging.info(f"mixed_blocks={mixed_blocks}, mixed_blobs={mixed_blobs}")
    logging.info(f"merged_blocks={merged_blocks}, merged_blobs={merged_blobs}")

    assert merged_blocks == merged_blocks_
    assert merged_blobs == merged_blobs_
    assert mixed_blocks == mixed_blocks_
    assert mixed_blobs == mixed_blobs_
...
def test_compaction():
    ...
    compaction_test(5000000, DEFAULT_BLOCK_COUNT, DEFAULT_BLOCK_COUNT / 1024, 0, 0)

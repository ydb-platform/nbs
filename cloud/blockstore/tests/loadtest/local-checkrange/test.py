...
class TestCase(object):
    ...
    def __init__(
        ...
        if storage_media_kind == "mirror2" or storage_media_kind == "mirror3":
            self.is_mirror = True
...
def __validate_checkrange(test_case, checkrange_result_file):
    ...
    __check_base_expectations(test_case, json_data)
    __check_expected_fields_content(
        test_case,
        json_data,
        expected_problem_ranges)
    __check_problem_ranges_merge_algorithm(json_data, expected_problem_ranges)


def __check_base_expectations(test_case, json_data):
    ...
    if json_data['Summary']['RequestsNum'] != expected_requests_num:
        raise Exception(f"Expected RequestsNum {expected_requests_num}, "
                        f"actual {json_data['Summary']['RequestsNum']}")
    if test_case.is_mirror and json_data['Summary']['ErrorsNum'] == 0:
        raise Exception("Expecting non zero 'Summary':'ErrorsNum'")


def __check_expected_fields_content(
        test_case,
        json_data,
        expected_problem_ranges):
    ...
    for r in json_data['Ranges']:
        ...
        if len(inconsistent_checksums) == 0:
            raise Exception(
                ...
                f"{start}-{end}")
        if len(inconsistent_checksums) != test_case.agent_count:
            raise Exception(
                f"Expected {test_case.agent_count} InconsistentChecksums for"
                f" {test_case.name}, actual {len(inconsistent_checksums)}")
